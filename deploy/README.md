...
curl -sSL https://raw.githubusercontent.com/thornboo/sub2api/dev-zz/deploy/docker-deploy.sh -o docker-deploy.sh
...
**Recommendation:** Use `docker-compose.local.yml` for easier data management and migration. The one-click `docker-deploy.sh` script downloads this local-directory version and saves it as `docker-compose.yml`.
...
1. Download the latest release from [GitHub Releases](https://github.com/thornboo/sub2api/releases)
