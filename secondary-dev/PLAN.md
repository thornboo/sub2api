...
Status: implemented and verified for API-key forwarding paths

Current implementation status:

- Implemented for the platform API-key forwarding paths covered by this demand:
  Anthropic/Claude API-key forwarding, Anthropic passthrough `count_tokens`,
  OpenAI API-key passthrough Responses and images paths, Gemini messages-compat
  API-key forwarding, Antigravity API-key/upstream forwarding, and Bedrock
  `auth_mode=apikey` forwarding.
...
- Legacy single-key accounts remain supported. A child-key pool is used only
  when the account has configured `account_api_keys`.
- Frontend create/edit forms now expose the upstream API-key pool for API-key
  account types on Anthropic, OpenAI, Gemini, Antigravity, Antigravity upstream,
  and Bedrock API-key mode while still allowing legacy single-key input.
