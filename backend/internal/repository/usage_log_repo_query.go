...
const usageLogSelectColumns = "id, user_id, api_key_id, account_id, request_id, model, requested_model, upstream_model, upstream_response_model, upstream_model_mismatch, group_id, member_id, member_code_snapshot, member_name_snapshot, subscription_id, input_tokens, output_tokens, cache_creation_tokens, cache_read_tokens, cache_creation_5m_tokens, cache_creation_1h_tokens, image_output_tokens, image_output_cost, image_input_tokens, image_input_cost, input_cost, output_cost, cache_creation_cost, cache_read_cost, total_cost, actual_cost, rate_multiplier, account_rate_multiplier, billing_type, request_type, stream, openai_ws_mode, duration_ms, first_token_ms, user_agent, ip_address, image_count, image_size, image_input_size, image_output_size, image_size_source, image_size_breakdown, video_count, video_resolution, video_duration_seconds, service_tier, reasoning_effort, requested_reasoning_effort, inbound_endpoint, upstream_endpoint, schedule_meta, route_plan_source, route_plan_snapshot_age_ms, cache_ttl_overridden, long_context_billing_applied, channel_id, model_mapping_chain, billing_tier, billing_mode, account_stats_cost, upstream_cost_binding_id, upstream_group_multiplier, upstream_price_reference_currency, upstream_reference_fx_rate, upstream_expected_cost, upstream_request_id, session_id, native_compaction_v2, created_at"
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		memberCodeSnapshot        sql.NullString
		memberNameSnapshot        sql.NullString
		...
		routePlanSource           sql.NullString
		routePlanSnapshotAgeMs    sql.NullInt64
		...
		upstreamCostBindingID     sql.NullInt64
		upstreamGroupMultiplier   sql.NullFloat64
		upstreamPriceCurrency     sql.NullString
		upstreamReferenceFXRate   sql.NullFloat64
		upstreamExpectedCost      sql.NullFloat64
	...
	if err := scanner.Scan(
		...
		&billingMode,
		&accountStatsCost,
		&upstreamCostBindingID,
		&upstreamGroupMultiplier,
		&upstreamPriceCurrency,
		&upstreamReferenceFXRate,
		&upstreamExpectedCost,
		&upstreamRequestID,
		&sessionID,
		&nativeCompactionV2,
		&createdAt,
	...
	if memberCodeSnapshot.Valid {
		log.MemberCodeSnapshot = &memberCodeSnapshot.String
	}
	if memberNameSnapshot.Valid {
		log.MemberNameSnapshot = &memberNameSnapshot.String
	}
	...
	if routePlanSource.Valid {
		log.RoutePlanSource = strings.TrimSpace(routePlanSource.String)
	}
	if routePlanSnapshotAgeMs.Valid {
		value := routePlanSnapshotAgeMs.Int64
		log.RoutePlanSnapshotAgeMs = &value
	}
	...
	if upstreamCostBindingID.Valid {
		log.UpstreamCostBindingID = &upstreamCostBindingID.Int64
	}
	if upstreamGroupMultiplier.Valid {
		log.UpstreamGroupMultiplier = &upstreamGroupMultiplier.Float64
	}
	if upstreamPriceCurrency.Valid {
		log.UpstreamPriceReferenceCurrency = &upstreamPriceCurrency.String
	}
	if upstreamReferenceFXRate.Valid {
		log.UpstreamReferenceFXRate = &upstreamReferenceFXRate.Float64
	}
	if upstreamExpectedCost.Valid {
		log.UpstreamExpectedCost = &upstreamExpectedCost.Float64
	}
	if sessionID.Valid {
