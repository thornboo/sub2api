...
func TestGetOwnerMemberAnalyticsLeaderboardReturnsOnlyRealMemberScope(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)member_scope AS \(\s*SELECT em\.id AS member_id\s*FROM enterprise_members em\s*WHERE em\.enterprise_user_id = \$\d+ AND em\.removed_at IS NULL\s*\),\s*ranked AS.*AND current_used_usd >= monthly_limit_usd \* 0\.8`).WillReturnRows(rows)
