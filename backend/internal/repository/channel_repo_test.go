...
func TestListModelPricingRoundTripsTimePricing(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT id, channel_id, sort_order, platform, models, billing_mode, input_price, output_price, cache_write_price, cache_write_1h_price, cache_read_price, fast_multiplier, flex_multiplier, max_reasoning_effort_multiplier, image_input_price, image_output_price, per_request_price, time_pricing, created_at, updated_at
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			1e-6, 2e-6, nil, nil, nil, nil, nil, nil, nil, nil, nil, timePricingJSON, now, now))
...
func TestTimePricingJSONPreservesDisabledSchedule(t *testing.T) {
	...
	restored, err := unmarshalTimePricing(encoded)
	require.NoError(t, err)
...
func TestTimePricingJSONMalformedReturnsError(t *testing.T) {
	restored, err := unmarshalTimePricing([]byte(`{"enabled":`))
	require.Error(t, err)
	require.Nil(t, restored)
	require.Contains(t, err.Error(), "unmarshal time pricing")
}
