...
func (s *AccountTestService) FetchUpstreamModelCatalog(ctx context.Context, account *Account) ([]UpstreamModelDescriptor, error) {
	catalog, _, err := s.fetchUpstreamModelList(ctx, account)
	return catalog, err
}
...
func (s *AccountTestService) SyncUpstreamModelCatalog(ctx context.Context, account *Account) (*UpstreamModelCatalog, error) {
	descriptors, body, err := s.fetchUpstreamModelList(ctx, account)
	models := upstreamModelCatalogIDs(descriptors)
	if err != nil {
	...
	if len(body) > 0 {
		_, directMetadata, parseErr := extractUpstreamModelMetadataCatalog(body, account != nil && account.IsGrok())
		if parseErr == nil {
...
func (s *AccountTestService) fetchUpstreamModelList(ctx context.Context, account *Account) ([]UpstreamModelDescriptor, []byte, error) {
	if s == nil {
...
func extractUpstreamModelCatalogWithSelector(body []byte, selectID func(upstreamModelEntry) string) ([]UpstreamModelDescriptor, error) {
	rawEntries, err := extractUpstreamModelRawEntries(body)
	if err != nil {
		return nil, err
	}
	...
	byID := make(map[string]*aggregate, len(rawEntries))
...
func extractUpstreamModelMetadataCatalog(body []byte, grok bool) ([]string, map[string]UpstreamModelMetadata, error) {
