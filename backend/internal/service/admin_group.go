...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	// 账号绑定发生在创建之后；固定账号 manifest 只能在创建后的编辑中开启。
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	if disableKeysForRateChange {
		err = s.runRateChangeTx(ctx, func(txCtx context.Context) error {
			...
			_, err := disabler.DisableKeysForGroupRateChange(txCtx, id)
			return err
		})
	} else {
...
func normalizeGroupModelPricing(platform string, pricing []ChannelModelPricing) ([]ChannelModelPricing, error) {
	...
	for i := range pricing {
		...
		out[i].ID = 0
		out[i].ChannelID = 0
		if out[i].TimePricing != nil && out[i].TimePricing.Enabled {
			return nil, infraerrors.BadRequest(
				"GROUP_MODEL_TIME_PRICING_UNSUPPORTED",
				"group model pricing does not support time pricing",
			)
		}
		// Group pricing is not a configuration surface for customer time pricing.
		// Tolerate and discard an empty disabled object from upstream-compatible clients.
...
func (s *adminServiceImpl) ClearGroupRateMultipliers(ctx context.Context, groupID int64) error {
	...
	err := s.runRateChangeTx(ctx, func(txCtx context.Context) error {
		...
		if len(affectedUserIDs) > 0 {
			_, err := disabler.DisableKeysForGroupUsersRateChange(txCtx, groupID, affectedUserIDs)
			return err
		}
		return nil
	})
