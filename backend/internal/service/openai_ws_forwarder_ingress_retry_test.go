...
type openAIWSPingFailCaptureConn struct {
	*openAIWSCaptureConn
	failWhenDrained bool
}

func (c *openAIWSPingFailCaptureConn) Ping(context.Context) error {
	if c == nil || c.openAIWSCaptureConn == nil {
		return errors.New("preflight ping failed")
	}
	if c.failWhenDrained {
		c.mu.Lock()
		drained := len(c.events) == 0
		c.mu.Unlock()
		if !drained {
			return nil
		}
...
// 场景：池内两条空闲连接同批陈旧。第二会话在 dispatch 前用 preflight ping
// 判定空闲连接失效后，必须新建连接，而不是再从池里拿另一条同样陈旧的连接。
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PreflightStaleIdleForcesFreshConn(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	// 首会话用完 events 后归还池内；第二会话借出池内空闲连接时，写前 preflight ping
	// 明确判定 stale，必须强制新建连接，不能再从同批空闲连接里继续挑。
	staleA := &openAIWSPingFailCaptureConn{
		openAIWSCaptureConn: &openAIWSCaptureConn{
			events: [][]byte{
	...
	staleB := &openAIWSPingFailCaptureConn{openAIWSCaptureConn: &openAIWSCaptureConn{}}
	...
	require.Equal(t, 2, dialer.DialCount(), "preflight 判定空闲连接失效后应强制新建连接")
	staleB.mu.Lock()
	staleWrites := len(staleB.writes)
	staleB.mu.Unlock()
	require.Equal(t, 0, staleWrites, "preflight 失败发生在 dispatch 前，不得向 stale 连接写入请求")
	fresh.mu.Lock()
