...
// Label 是管理员配置的客户可见规则文案；同倍率但不同 Label 的显式规则仍需保留。
...
// TimePricingSchedule 分组+模型生效的分时倍率（仅列出显式规则，按开始时间升序）。
// WeekdaysOnly 为 true 时显式规则仅周一至周五生效，周末整天按默认倍率和标签计费。
...
func (s *BillingService) ResolveContextPricingSchedule(ctx context.Context, resolver *ModelPricingResolver, in ContextPricingScheduleInput) (*ContextPricingSchedule, error) {
	...
	tierResolved.TimePricing = nil
	probe := func(tokens UsageTokens) (*CostBreakdown, error) {
		r := req
		r.Tokens = tokens
		r.Resolved = &tierResolved
...
// 时段来自解析到的渠道定价配置，每个时段的倍率用 TimePricing.MultiplierAt
// 在时段内取值；定价来源不是渠道（分组价卡覆盖）或配置非法时不列分时。
func resolvedTimePricingSchedule(resolved *ResolvedPricing) *TimePricingSchedule {
	...
	if _, err := validateTimePricing(cfg); err != nil {
		return nil
	}
	defaultApplied := cfg.defaultApplied()
	location, err := time.LoadLocation(defaultApplied.Timezone)
	if err != nil {
		return nil
	}
	...
	probed := make([]probedPeriod, 0, len(cfg.Rules))
	for _, rule := range cfg.Rules {
		...
		// 锚点日必须是工作日（2026-01-05 为周一），否则 weekdays_only 会回到默认倍率。
		at := time.Date(2026, time.January, 5, start/60, start%60, 1, 0, location)
		applied := cfg.MultiplierAt(at)
		if applied.Rule == nil {
			continue
		}
		if applied.Multiplier == defaultApplied.Multiplier && applied.Label == defaultApplied.Label {
			continue
		}
	...
	if len(probed) == 0 && defaultApplied.Multiplier == 1 {
		return nil
	}
	sort.SliceStable(probed, func(i, j int) bool { return probed[i].start < probed[j].start })
	out := &TimePricingSchedule{
		Timezone:          defaultApplied.Timezone,
		WeekdaysOnly:      cfg.WeekdaysOnly,
		DefaultLabel:      defaultApplied.Label,
		DefaultMultiplier: defaultApplied.Multiplier,
