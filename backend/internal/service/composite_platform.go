...
// WithoutCompositeRouteDecision shadows any route decision inherited from a
// previous enterprise-member candidate. A composite decision is scoped to one
// concrete group attempt and must never leak into the next candidate.
