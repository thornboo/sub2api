...
func (s *OpenAIGatewayService) ForwardAsChatCompletionsWithSelectedProtocol(
	...
	return s.forwardAsChatCompletions(ctx, c, account, body, promptCacheKey, defaultMappedModel, selectedProtocol, false)
}
...
func (s *OpenAIGatewayService) forwardAsChatCompletions(
	...
	// For legacy callers without a canonical delivery decision, explicit CN
	// protocol configuration remains authoritative over asynchronous probe data.
	if selectedProtocol == "" && shouldForwardOpenAIResponsesViaRawChatCompletions(account) {
		return s.forwardAsRawChatCompletions(ctx, c, account, body, defaultMappedModel)
	}
	...
	if resp.StatusCode >= 400 {
		...
		if !agentIdentityTaskRecoveryWasTried(ctx) && s.isAgentIdentityAccount(ctx, account) && isAgentIdentityTaskInvalidHTTPResponse(resp.StatusCode, respBody) {
			...
			return s.forwardAsChatCompletions(markAgentIdentityTaskRecoveryTried(ctx), c, account, body, promptCacheKey, defaultMappedModel, selectedProtocol, compatPromptCacheTenantIsolated)
		}
		if account.Type == AccountTypeAPIKey &&
