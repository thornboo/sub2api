...
func TestPassthroughIngressLocalRejectionDoesNotForwardFollowUp(t *testing.T) {
	t.Run("before_request", func(t *testing.T) {
		rejection := errors.New("request rejected")
		runPassthroughIngressLocalRejectionDoesNotForwardFollowUp(t, rejection, func(afterTurn func(int, *OpenAIForwardResult, error)) *OpenAIWSIngressHooks {
			return &OpenAIWSIngressHooks{
				BeforeRequest: func(int, []byte, string) error { return rejection },
				AfterTurn:     afterTurn,
			}
		})
	})
	t.Run("before_turn", func(t *testing.T) {
		...
		require.Equal(t, 2, beforeTurnTurn)
	})
	t.Run("map_request_model", func(t *testing.T) {
		rejection := errors.New("map rejected")
		runPassthroughIngressLocalRejectionDoesNotForwardFollowUp(t, rejection, func(afterTurn func(int, *OpenAIForwardResult, error)) *OpenAIWSIngressHooks {
			return &OpenAIWSIngressHooks{
				MapRequestModel: func(turn int, _ string) (string, error) {
					if turn == 2 {
						return "", rejection
					}
					return "", nil
...
func runPassthroughIngressLocalRejectionDoesNotForwardFollowUp(t *testing.T, rejection error, hooksFor func(func(int, *OpenAIForwardResult, error)) *OpenAIWSIngressHooks) {
	t.Helper()
	gin.SetMode(gin.TestMode)
	controlCtx, cancelControl := context.WithCancelCause(context.Background())
	defer cancelControl(context.Canceled)
	...
	afterTurnCalls := 0
	var finalErr error
	hooks := hooksFor(func(_ int, _ *OpenAIForwardResult, turnErr error) {
		hooksMu.Lock()
		afterTurnCalls++
		if turnErr != nil {
			finalErr = turnErr
	...
	case <-time.After(3 * time.Second):
		t.Fatal("passthrough ingress did not exit after local rejection")
	}

	hooksMu.Lock()
	gotAfter, gotFinalErr := afterTurnCalls, finalErr
	hooksMu.Unlock()
	require.Equal(t, 2, gotAfter, "each started turn must be finalized exactly once")
	require.ErrorIs(t, gotFinalErr, rejection)
}

func TestPassthroughIngressPolicyBlockFinalizesTurnDoesNotForwardFollowUp(t *testing.T) {
	gin.SetMode(gin.TestMode)
	controlCtx, cancelControl := context.WithCancelCause(context.Background())
	defer cancelControl(context.Canceled)

	upstream := newStagedPassthroughConn()
	upstream.Send(`{"type":"response.completed","response":{"id":"resp_policy_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`)
	...
	svc.settingService = newOpenAIGatewayServiceWithSettings(t, settings).settingService
	...
	writeCtx, cancelWrite := context.WithTimeout(context.Background(), time.Second)
	err = clientConn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.1","service_tier":"priority"}`))
	cancelWrite()
	require.NoError(t, err)
	select {
	case payload := <-upstream.writes:
		t.Fatalf("policy-blocked response.create was forwarded upstream: %s", payload)
	case <-time.After(100 * time.Millisecond):
	}
	select {
	case err = <-serverErr:
		require.Error(t, err)
		require.Contains(t, err.Error(), "ws fast blocked")
	case <-time.After(5 * time.Second):
		t.Fatal("passthrough ingress did not exit after policy block")
	}

	hooksMu.Lock()
	gotAfter, gotFinalErr := afterTurnCalls, finalErr
	hooksMu.Unlock()
	require.Equal(t, 2, gotAfter, "policy-blocked turn must be finalized exactly once")
	require.Error(t, gotFinalErr)
	require.Contains(t, gotFinalErr.Error(), "ws fast blocked")
}
