...
readLoop:
	for {
		...
		if readErr == nil && !json.Valid(message) {
			...
			MarkEnterpriseMemberBudgetOutcomeAmbiguousWithReason(c, "upstream_ws_outcome_unknown")
			setOpsUpstreamError(c, 0, "upstream websocket returned malformed Responses event JSON", "")
			if !wroteDownstream {
				return nil, errors.New("upstream websocket returned malformed Responses event JSON")
			}
