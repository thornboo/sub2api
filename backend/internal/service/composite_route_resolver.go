...
// ListCatalogRoutes returns enabled exact routes that represent concrete,
// discoverable public model IDs. Prefix routes are matching rules rather than
// enumerable model identifiers and are intentionally excluded.
func (r *CompositeRouteResolver) ListCatalogRoutes(ctx context.Context, groupID int64) ([]CompositeModelRoute, error) {
	if r == nil || r.repo == nil || groupID <= 0 {
		return nil, nil
	}
	routes, err := r.repo.ListByGroup(ctx, groupID, false)
	if err != nil {
		return nil, fmt.Errorf("list composite catalog routes: %w", err)
	}
	result := make([]CompositeModelRoute, 0, len(routes))
