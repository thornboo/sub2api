...
func validateAccountStatsPricingRules(rules []AccountStatsPricingRule) error {
	for i := range rules {
		for j := range rules[i].Pricing {
			timePricing := rules[i].Pricing[j].TimePricing
			if timePricing == nil {
				continue
			}
			if timePricing.Enabled {
			...
			// Compatibility with upstream clients that send an empty disabled object:
			// account-cost snapshots never persist customer sale schedules.
			rules[i].Pricing[j].TimePricing = nil
		}
