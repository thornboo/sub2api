...
func openAIActualInputTokensForUsage(account *Account, usage OpenAIUsage) int {
	actualInputTokens := usage.InputTokens - usage.CacheCreationInputTokens
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	usageLog := &UsageLog{
		...
		RequestedReasoningEffort: func() *string {
			if result.RequestedReasoningEffort != nil {
				return result.RequestedReasoningEffort
			}
	...
	if resolvedGroupID != nil {
		applyAccountStatsCost(ctx, usageLog, s.channelService, s.billingService,
			account.ID, *resolvedGroupID, result.UpstreamModel, result.Model,
	...
	if billingErr != nil {
		// Enterprise member settlement and attribution must remain atomic. A
		// standalone unsettled log would persist member usage without the matching
		// member-budget transaction, so keep the fail-closed contract for members.
