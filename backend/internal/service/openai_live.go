...
func liveCopyID(value *int64) *int64 {
	if value == nil {
		return nil
	}
	result := *value
...
func liveSameOptionalID(left, right *int64) bool {
...
func (s *OpenAIGatewayService) CreateLiveCall(
	...
	for attempt := 0; attempt <= 3; attempt++ {
		...
		record := &LiveCallRecord{
			...
			MemberID:              liveCopyID(identity.MemberID),
			MemberCodeSnapshot:    optionalTrimmedStringPtr(identity.MemberCodeSnapshot),
			MemberNameSnapshot:    optionalTrimmedStringPtr(identity.MemberNameSnapshot),
...
func (s *OpenAIGatewayService) GetLiveCallForIdentity(
	...
	if record.CallID != callID ||
		...
		!liveSameOptionalID(record.MemberID, identity.MemberID) {
...
func (s *OpenAIGatewayService) writeLiveUsageLog(record *LiveCallRecord, usageLog *UsageLog) {
	if s == nil || s.usageLogRepo == nil || record == nil || usageLog == nil {
		return
	}
	usageCtx, cancel := detachedBillingContext(context.Background())
	defer cancel()
	...
	if writer, ok := repo.(usageLogBestEffortWriter); ok {
		...
		logLiveUsageInsertResult(record, inserted, err)
		return
	}
...
func logLiveUsageInsertResult(record *LiveCallRecord, inserted bool, err error) {
	if err != nil {
		logger.L().Error("openai_live.usage_log_insert_failed",
	...
	if !inserted {
		logger.L().Warn("openai_live.usage_log_not_inserted",
			zap.String("call_hash", record.CallHash),
			zap.Int64("account_id", record.AccountID),
			zap.Int64("api_key_id", record.APIKeyID),
			zap.Int64("user_id", record.UserID),
...
func (s *OpenAIGatewayService) finalizeLiveCall(record *LiveCallRecord) {
	...
	if record.MemberID != nil {
		usageLog.MemberID = liveCopyID(record.MemberID)
		usageLog.MemberCodeSnapshot = optionalTrimmedStringPtr(liveStringValue(record.MemberCodeSnapshot))
		usageLog.MemberNameSnapshot = optionalTrimmedStringPtr(liveStringValue(record.MemberNameSnapshot))
	}
	s.writeLiveUsageLog(record, usageLog)
}
