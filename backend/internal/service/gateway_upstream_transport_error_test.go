...
// TestHandleUpstreamTransportError_TransientStopsReplayWithoutEviction pins the
// fork contract for transient transport blips (EOF / connection reset): the
// current account stays schedulable, but an ambiguous upstream outcome cannot
// be replayed on another account. The handler still owns the response.
func TestHandleUpstreamTransportError_TransientStopsReplayWithoutEviction(t *testing.T) {
	...
	if failoverErr.ShouldRetryNextAccount() {
		t.Fatal("transient transport error with an ambiguous outcome must not replay on the next account")
	}
	if !IsEnterpriseMemberBudgetOutcomeAmbiguous(c) {
		t.Fatal("transient transport error must preserve the ambiguous budget outcome")
	}
	if repo.calls != 0 {
