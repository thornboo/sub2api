...
func TestEnterpriseMemberMetricsSnapshotUsesBoundedReasonsAndLifecycleCounters(t *testing.T) {
	...
	RecordEnterpriseMemberRoutePlanning(EnterpriseMemberModelAdmissionShadowPublished, []int64{11, 12}, []int64{12}, nil)
	RecordEnterpriseMemberRoutePlanning(EnterpriseMemberModelAdmissionShadowPublished, []int64{11, 12}, []int64{11, 12}, nil)
	RecordEnterpriseMemberRoutePlanning(EnterpriseMemberModelAdmissionEnforcePublished, []int64{11}, nil, errors.New("projection unavailable"))
	RecordEnterpriseMemberRoutePlanning(EnterpriseMemberModelAdmissionLegacyOrderOnly, []int64{11}, []int64{11}, nil)
	...
	require.Equal(t, uint64(3), snapshot.RoutePlanningEvaluationTotal)
	require.Equal(t, uint64(2), snapshot.RoutePlanningShadowTotal)
	require.Equal(t, uint64(1), snapshot.RoutePlanningEnforceTotal)
	require.Equal(t, uint64(1), snapshot.RoutePlanningErrorTotal)
	require.Equal(t, uint64(1), snapshot.RouteShadowDiffTotal)
	require.Equal(t, uint64(1), snapshot.RoutePlanningLiveTotal)
	require.Equal(t, uint64(1), snapshot.RoutePlanningLKGHitTotal)
	require.Equal(t, uint64(1), snapshot.RoutePlanningLKGMissTotal)
	require.Equal(t, uint64(1), snapshot.RoutePlanningLegacyEffectiveTotal)
	require.Equal(t, uint64(1), snapshot.RoutingRevisionUnavailableTotal)
