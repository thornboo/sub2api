...
func (s *adminServiceImpl) UpdateUser(ctx context.Context, id int64, input *UpdateUserInput) (*User, error) {
	...
	if input.AccountType != "" || input.EnterpriseEnabled != nil {
		fields.EnterpriseDisabledAt = true
	}
	...
	if s.authCacheInvalidator != nil {
		...
		if user.Concurrency != oldConcurrency || user.Status != oldStatus || user.Role != oldRole || user.AccountType != oldAccountType || !sameOptionalTime(user.EnterpriseDisabledAt, oldEnterpriseDisabledAt) || user.RPMLimit != oldRPMLimit || user.RestrictPublicGroups != oldRestrictPublicGroups || !sameInt64Set(user.AllowedGroups, oldAllowedGroups) || groupRatesTouched {
