...
func ProvideRateLimitService(
	...
	svc.SetModelFailCounterCache(modelFailCounterCache)
...
// 启用状态由 settings 服务在运行时解析，因此后台改开关后无需重启即可生效；任务存储、
// 历史查询和企业成员预算恢复始终保持可用，避免配置回退时留下无法收敛的预算占用。
func ProvideImageTaskService(store ImageTaskStore, settings *ImageStorageSettingService, memberBudget *EnterpriseMemberBudgetService) *ImageTaskService {
...
var ProviderSet = wire.NewSet(
	...
	ProvideEnterpriseMemberBudgetService,
	NewEnterpriseMemberImportService,
	ProvideEnterpriseMemberImportWorker,
	ProvideEnterpriseMemberImportCleanupService,
	ProvideEnterpriseMemberBudgetRecoveryService,
	...
	ProvideUpstreamSupplierBalancePoller,
...
func ProvideUpstreamSupplierBalancePoller(db *dbent.Client, cfg *config.Config, encryptor SecretEncryptor) *UpstreamSupplierBalancePoller {
	poller := NewUpstreamSupplierBalancePoller(db, cfg, encryptor)
	poller.Start()
	return poller
}
