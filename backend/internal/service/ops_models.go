...
type OpsErrorLogFilter struct {
	...
	StatusCodes        []int
	StatusCodesExclude []int
	StatusCodesOther   bool
	Phase              string // Recovered provider rows bypass status>=400 only with IncludeRecoveredUpstream.
	Owner              string
	Source             string
	Resolved           *bool
	Query              string
	...
	// single Phase field. With IncludeRecoveredUpstream, an ANY list containing
	// only upstream/account_auth also bypasses the status>=400 guard.
