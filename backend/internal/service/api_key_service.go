	...
	err = txRepo.RunInTx(ctx, func(txCtx context.Context) error {
		for _, id := range ids {
			...
			fields := APIKeyUpdateFields{
				Tags:           req.UpdateTags,
				Status:         req.UpdateStatus || key.Status != originalStatus,
				Quota:          req.UpdateQuota,
				GroupID:        req.UpdateGroup,
				ExpiresAt:      req.UpdateExpiration,
				RateLimits:     req.UpdateRateLimit,
				RateLimitUsage: req.ResetRateLimitUsage,
				IPRules:        req.UpdateIPAccessControl,
			}
			if err := s.apiKeyRepo.Update(txCtx, &key, fields); err != nil {
