...
func sonnetChannelWithTimePricing(tp *TimePricing) []ChannelModelPricing {
	return []ChannelModelPricing{{
		Platform: PlatformAnthropic, Models: []string{"claude-sonnet-4"}, BillingMode: BillingModeToken,
...
func TestResolveContextPricingSchedule_TimePricing(t *testing.T) {
	valid := &TimePricing{
		...
		Rules: []TimePricingRule{
			{Label: "evening", StartTime: "18:00", EndTime: "22:00", Multiplier: 1.2},
			{Label: "night", StartTime: "00:30", EndTime: "08:30", Multiplier: 0.5},
			{Label: "midday", StartTime: "12:00", EndTime: "13:00", Multiplier: 1},
		},
	}

	t.Run("渠道分时按开始时间升序列出且保留客户可见规则标签", func(t *testing.T) {
		...
		require.Equal(t, "standard", sched.TimePricing.DefaultLabel)
		require.Equal(t, 1.0, sched.TimePricing.DefaultMultiplier)
	...
	t.Run("仅工作日配置透传标注且时段仍列出", func(t *testing.T) {
		...
		weekdays := &TimePricing{
			Enabled:           true,
			Timezone:          "Asia/Shanghai",
			WeekdaysOnly:      true,
			DefaultLabel:      "weekend-standard",
		...
		require.Equal(t, "weekend-standard", sched.TimePricing.DefaultLabel)
		require.Equal(t, 0.8, sched.TimePricing.DefaultMultiplier)
		require.Equal(t, []TimePricingPeriod{
			{Label: "night", StartTime: "00:30", EndTime: "08:30", Multiplier: 0.5},
		}, sched.TimePricing.Periods)

		// 对账：工作日时段内乘规则倍率，周末同一时段回到 default 倍率和标签
		group := enabledGroup(PlatformAnthropic)
		gid := group.ID
		resolved := resolver.Resolve(context.Background(), PricingInput{Model: "claude-sonnet-4", GroupID: &gid, Group: group})
		loc, err := time.LoadLocation("Asia/Shanghai")
		...
		for _, tc := range []struct {
			...
			{time.Date(2026, 8, 23, 3, 0, 0, 0, loc), 0.8}, // 周日
		} {
			cost, err := bs.CalculateTokenCostForRequest(TokenCostRequest{
				Ctx: context.Background(), Model: "claude-sonnet-4", Group: group, Tokens: UsageTokens{InputTokens: 1000},
				RateMultiplier: 1, PricingAt: tc.at, Resolver: resolver, Resolved: resolved,
		...
		weekendCost, err := bs.CalculateTokenCostForRequest(TokenCostRequest{
			Ctx: context.Background(), Model: "claude-sonnet-4", Group: group, Tokens: UsageTokens{InputTokens: 1000},
			RateMultiplier: 1, PricingAt: time.Date(2026, 8, 23, 3, 0, 0, 0, loc), Resolver: resolver, Resolved: resolved,
		})
		require.NoError(t, err)
		require.Equal(t, "weekend-standard", weekendCost.TimePricingLabel)
		require.Nil(t, weekendCost.TimePricingRule)
	})
	...
	t.Run("配置非法时计费按 1 计，阶梯表不列分时", func(t *testing.T) {
		invalid := &TimePricing{
			...
			Rules: []TimePricingRule{
				{Label: "night", StartTime: "00:30", EndTime: "08:30", Multiplier: 0.5},
				{Label: "overlap", StartTime: "08:00", EndTime: "09:00", Multiplier: 0.8}, // 与上一段重叠
			},
		}
