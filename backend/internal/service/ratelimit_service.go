...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldFailover bool) {
	...
	// A definite upstream model-not-found response has stronger semantics than
	// a broad custom keyword rule and must keep its dedicated model cooldown.
	if len(requestedModel) > 0 && !isAnthropic429 &&
		s.HandleUpstreamModelNotFound(ctx, account, requestedModel[0], statusCode, responseBody) {
		return true
	}

	// Explicit administrator rules come before the generic provider fallback.
	// With a known model they isolate only account+model; otherwise they retain
	// the historical account-level temporary-unschedulable behavior.
