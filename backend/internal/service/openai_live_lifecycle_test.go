...
func (s *liveLogSink) WriteLogEvent(event *logger.LogEvent) {
	s.mu.Lock()
	defer s.mu.Unlock()
	copy := *event
	if event.Fields != nil {
...
func (s *liveLogSink) find(message string) *logger.LogEvent {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func TestFinalizeLiveCallIsIdempotentAndWritesZeroUsage(t *testing.T) {
	memberID := int64(55)
	memberCode := "member-55"
	memberName := "Member 55"
	record := &LiveCallRecord{
		CallID:             "call_secret",
	...
	require.NotNil(t, log.MemberCodeSnapshot)
	require.NotNil(t, log.MemberNameSnapshot)
	require.Equal(t, memberCode, *log.MemberCodeSnapshot)
	require.Equal(t, memberName, *log.MemberNameSnapshot)
}

func TestFinalizeLiveCallLogsUsageInsertFailure(t *testing.T) {
	sink := &liveLogSink{}
	require.NoError(t, logger.Init(logger.InitOptions{
		Level:       "error",
		Format:      "json",
		ServiceName: "sub2api-test",
		Environment: "test",
	...
	record := &LiveCallRecord{
		CallID:     "call_secret_failure",
		CallHash:   hashLiveCallID("call_secret_failure"),
		AccountID:  11,
		APIKeyID:   22,
		UserID:     33,
		GroupID:    44,
	...
	service := &OpenAIGatewayService{
		...
		usageLogRepo:       &liveTestUsageRepo{createErr: errors.New("usage insert unavailable")},
	}

	service.finalizeLiveCall(record)

	event := sink.find("openai_live.usage_log_insert_failed")
	require.NotNil(t, event)
	require.Equal(t, record.CallHash, event.Fields["call_hash"])
	require.Equal(t, record.AccountID, event.Fields["account_id"])
	require.Equal(t, record.APIKeyID, event.Fields["api_key_id"])
	require.Equal(t, record.UserID, event.Fields["user_id"])
	require.NotContains(t, event.Fields, "call_id")
}
...
func TestGetLiveCallForIdentityRejectsMismatchedCaller(t *testing.T) {
	...
	memberID := int64(55)
	otherMemberID := int64(66)
	memberRecord := *record
	memberRecord.CallID = "call_member_identity"
	memberRecord.CallHash = hashLiveCallID(memberRecord.CallID)
	memberRecord.MemberID = &memberID
	require.NoError(t, store.SaveLiveCall(context.Background(), &memberRecord, time.Hour))

	for name, callerMemberID := range map[string]*int64{
		"different member": &otherMemberID,
		"missing member":   nil,
	} {
		t.Run(name, func(t *testing.T) {
			_, err := service.GetLiveCallForIdentity(context.Background(), memberRecord.CallID, LiveCallIdentity{
				APIKeyID: memberRecord.APIKeyID,
	...
	loaded, err = service.GetLiveCallForIdentity(context.Background(), memberRecord.CallID, LiveCallIdentity{
		APIKeyID: memberRecord.APIKeyID,
		UserID:   memberRecord.UserID,
		GroupID:  &groupID,
		MemberID: &memberID,
	...
	require.NoError(t, err)
	require.Equal(t, memberID, *loaded.MemberID)
}
