...
func TestCompositeRouteResolverCatalogIncludesOnlyEnabledExactAliases(t *testing.T) {
	resolver := NewCompositeRouteResolver(compositeRouteRepoStub{
		routes: []CompositeModelRoute{
			{ID: 1, GroupID: 7, PublicModel: "all/gpt", MatchType: CompositeRouteMatchExact, TargetPlatform: PlatformOpenAI, Endpoint: CompositeRouteEndpointResponses, Enabled: true},
			{ID: 2, GroupID: 7, PublicModel: "all/", MatchType: CompositeRouteMatchPrefix, TargetPlatform: PlatformOpenAI, Endpoint: CompositeRouteEndpointAny, Enabled: true},
			{ID: 3, GroupID: 7, PublicModel: "disabled/alias", MatchType: CompositeRouteMatchExact, TargetPlatform: PlatformOpenAI, Endpoint: CompositeRouteEndpointAny, Enabled: false},
			{ID: 4, GroupID: 8, PublicModel: "other-group", MatchType: CompositeRouteMatchExact, TargetPlatform: PlatformOpenAI, Endpoint: CompositeRouteEndpointAny, Enabled: true},
		},
	})

	routes, err := resolver.ListCatalogRoutes(context.Background(), 7)

	require.NoError(t, err)
	require.Len(t, routes, 1)
	require.Equal(t, "all/gpt", routes[0].PublicModel)
	require.Equal(t, CompositeRouteEndpointResponses, routes[0].Endpoint)
}
