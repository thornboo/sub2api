...
// CheckGroupModelAllowlist validates the current request against the currently
// active API key group. Enterprise member orchestration calls this for every
// activated candidate group, so a later candidate cannot inherit the bootstrap
// group's allowlist decision.
func CheckGroupModelAllowlist(c *gin.Context) bool {
