...
func registerGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	groups := admin.Group("/groups")
	{
		...
		groups.GET("/:id/messages-dispatch-model-candidates", h.Admin.Group.GetMessagesDispatchModelCandidates)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/:id/clear-model-rate-limit", h.Admin.Account.ClearModelRateLimit)
...
func registerUpstreamCostPoolRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	suppliers := admin.Group("/upstream-suppliers")
	{
		...
		suppliers.GET("/recharge-overview", h.Admin.Account.GetUpstreamSupplierRechargeOverview)
		suppliers.POST("", h.Admin.Account.CreateUpstreamSupplier)
		suppliers.POST("/:supplier_id/balance/refresh", h.Admin.Account.RefreshUpstreamSupplierBalance)
		suppliers.GET("/:supplier_id/recharge-trend", h.Admin.Account.GetUpstreamSupplierRechargeTrend)
...
func registerFeedbackRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	{
		feedback.GET("", h.Admin.Feedback.List)
		feedback.GET("/:id", h.Admin.Feedback.Get)
		feedback.GET("/:id/messages", h.Admin.Feedback.ListMessages)
		feedback.POST("/:id/messages", h.Admin.Feedback.Reply)
		feedback.POST("/:id/read", h.Admin.Feedback.MarkRead)
		feedback.POST("/:id/close", h.Admin.Feedback.Close)
	}
}
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		// 模型级限流策略配置
		adminSettings.GET("/model-rate-limit", h.Admin.Setting.GetModelRateLimitSettings)
		adminSettings.PUT("/model-rate-limit", h.Admin.Setting.UpdateModelRateLimitSettings)
