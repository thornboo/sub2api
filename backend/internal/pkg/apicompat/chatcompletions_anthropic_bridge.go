...
func NewChatCompletionsToAnthropicStreamState(model string) *ChatCompletionsToAnthropicStreamState {
	return &ChatCompletionsToAnthropicStreamState{
		...
		toolArgumentBytes: make(map[int]int),
		maxToolArgBytes:   defaultMaxChatStreamToolArgumentBytes,
		maxTotalToolBytes: defaultMaxChatStreamTotalToolArgumentBytes,
	}
}
...
func (state *ChatCompletionsToAnthropicStreamState) StreamError() error {
	if state == nil {
		return nil
	}
...
func (state *ChatCompletionsToAnthropicStreamState) recordToolArguments(index int, delta string) error {
	if delta == "" {
		return nil
	}
	currentBytes := state.toolArgumentBytes[index]
	if len(delta) > state.maxToolArgBytes-currentBytes {
		return fmt.Errorf("chat completions upstream tool arguments exceed %d bytes for one call", state.maxToolArgBytes)
	}
	if len(delta) > state.maxTotalToolBytes-state.totalToolArgBytes {
		return fmt.Errorf("chat completions upstream tool arguments exceed %d total bytes", state.maxTotalToolBytes)
	}
	state.toolArgumentBytes[index] = currentBytes + len(delta)
	state.totalToolArgBytes += len(delta)
	return nil
}
...
func ChatCompletionsChunkToAnthropicEvents(
	...
	for _, choice := range chunk.Choices {
		...
		for _, toolCall := range choice.Delta.ToolCalls {
			...
			if err := state.recordToolArguments(idx, toolCall.Function.Arguments); err != nil {
...
func FinalizeChatCompletionsAnthropicStream(state *ChatCompletionsToAnthropicStreamState) []AnthropicStreamEvent {
	if state == nil || state.MessageStopSent || state.streamError != nil {
		return nil
	}
