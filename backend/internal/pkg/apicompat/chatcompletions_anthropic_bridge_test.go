...
func TestChatCompletionsToAnthropicStreamState_RejectsPerCallToolArgumentLimit(t *testing.T) {
	state := NewChatCompletionsToAnthropicStreamState("gpt-5.4")
	state.maxToolArgBytes = 8
	state.maxTotalToolBytes = 16
	idx := 0
	...
	require.Contains(t, state.StreamError().Error(), "exceed 8 bytes")
	require.Equal(t, []string{"message_start"}, anthropicEventTypes(events))
	require.Empty(t, FinalizeChatCompletionsAnthropicStream(state), "limit failures must not emit a normal message_stop")
}

func TestChatCompletionsToAnthropicStreamState_RejectsTotalToolArgumentLimit(t *testing.T) {
	state := NewChatCompletionsToAnthropicStreamState("gpt-5.4")
	state.maxToolArgBytes = 8
	state.maxTotalToolBytes = 10
	first, second := 0, 1

	_ = ChatCompletionsChunkToAnthropicEvents(&ChatCompletionsChunk{
		Choices: []ChatChunkChoice{{Delta: ChatDelta{ToolCalls: []ChatToolCall{
	...
	require.Error(t, state.StreamError())
	require.Contains(t, state.StreamError().Error(), "exceed 10 total bytes")
	require.Equal(t, 6, state.toolArgumentBytes[first])
	require.Zero(t, state.toolArgumentBytes[second], "rejected fragments must not enter the aggregate counters")
	require.Empty(t, FinalizeChatCompletionsAnthropicStream(state), "limit failures must not emit a normal message_stop")
}
