...
func ResponsesToChatCompletionsRequestWithOptions(req *ResponsesRequest, opts *ResponsesToChatOptions) (*ChatCompletionsRequest, error) {
	...
	return ResponsesToChatCompletionsRequestWithRegistryAndOptions(req, registry, DefaultChatCompletionsCapabilities(), opts)
}
...
func ResponsesToChatCompletionsRequestWithRegistry(req *ResponsesRequest, registry *ResponsesToolRegistry, capabilities ChatCompletionsCapabilities) (*ChatCompletionsRequest, error) {
	return ResponsesToChatCompletionsRequestWithRegistryAndOptions(req, registry, capabilities, nil)
}

// ResponsesToChatCompletionsRequestWithRegistryAndOptions combines the
// request-local tool registry/capability contract with optional history hooks.
func ResponsesToChatCompletionsRequestWithRegistryAndOptions(req *ResponsesRequest, registry *ResponsesToolRegistry, capabilities ChatCompletionsCapabilities, opts *ResponsesToChatOptions) (*ChatCompletionsRequest, error) {
	if req == nil {
	...
	messages, err := responsesInputToChatMessagesWithRegistryAndOptions(req.Instructions, registry, capabilities, opts)
	if err != nil {
		return nil, err
	}
...
func responsesInputToChatMessagesWithRegistry(instructions string, registry *ResponsesToolRegistry, capabilities ChatCompletionsCapabilities) ([]ChatMessage, error) {
	return responsesInputToChatMessagesWithRegistryAndOptions(instructions, registry, capabilities, nil)
}

func responsesInputToChatMessagesWithRegistryAndOptions(instructions string, registry *ResponsesToolRegistry, capabilities ChatCompletionsCapabilities, opts *ResponsesToChatOptions) ([]ChatMessage, error) {
	...
	built, mediaByCallID, err := buildChatMessagesFromItems(messages, registry.inputItems, registry, capabilities, opts)
	if err != nil {
		return nil, err
	}
...
func buildChatMessagesFromItems(messages []ChatMessage, rawItems []json.RawMessage, registry *ResponsesToolRegistry, capabilities ChatCompletionsCapabilities, opts *ResponsesToChatOptions) ([]ChatMessage, toolOutputMediaByCallID, error) {
	...
	for inputItemIndex, raw := range rawItems {
		...
		case "tool_search_call":
			...
			callID := rawString(item["call_id"])
			if callID == "" {
				return nil, nil, fmt.Errorf("client tool_search_call is missing call_id")
			}
		...
		case "tool_search_output":
			...
			callID := rawString(item["call_id"])
			if callID == "" {
				return nil, nil, fmt.Errorf("tool_search_output without call_id cannot be represented as a Chat Completions tool result")
			}
			...
			if len(toolsRaw) == 0 || string(toolsRaw) == "null" {
				...
				if len(outputRaw) == 0 || string(outputRaw) == "null" {
					return nil, nil, fmt.Errorf("tool_search_output %q is missing tools or output", callID)
				}
				...
				if rewritten {
					...
					if outputText == "" && string(outputRaw) != `""` {
...
func (state *ChatCompletionsToResponsesStreamState) ValidateToolCallArguments() error {
	...
	for idx, toolCall := range state.ToolCalls {
		...
		arguments := strings.TrimSpace(state.toolArgumentsFor(idx))
		if arguments == "" {
			continue
		}
		if !json.Valid([]byte(arguments)) {
