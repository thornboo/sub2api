...
func TestResponsesToolOutputMedia_ExtractsSupportedShapes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			call:       `{"type":"tool_search_call","execution":"client","call_id":"call_image","arguments":{"query":"image"}}`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			input := fmt.Sprintf(`[%s,{"type":%q,"execution":"client","call_id":"call_image","output":%s}]`, tt.call, tt.outputType, tt.output)
...
func TestResponsesToChatCompletionsRequest_ToolContentNeverContainsExtractedMedia(t *testing.T) {
	req := &ResponsesRequest{
		...
		Input: json.RawMessage(`[
			...
			{"type":"tool_search_call","execution":"client","call_id":"call_search","arguments":{"query":"image"}},
			...
			{"type":"tool_search_output","execution":"client","call_id":"call_search","output":"data:image/jpeg;base64,BAUG"}
		]`),
	}
