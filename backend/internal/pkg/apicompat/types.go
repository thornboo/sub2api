...
type ResponsesTool struct {
	...
	// rawDefinition preserves fields this gateway does not model yet. It is used
	// only for duplicate-definition comparison so future Responses tool fields
	// cannot be silently discarded before conflict detection.
...
func (t *ResponsesTool) UnmarshalJSON(data []byte) error {
	...
	*t = ResponsesTool(a)
	t.rawDefinition = append(json.RawMessage(nil), bytesTrimSpace(data)...)
	return nil
}
