...
type GatewayConfig struct {
	...
	// EnterpriseMemberModelAdmissionMode controls how enterprise-member
	// multi-group routing uses exact published model eligibility.
	EnterpriseMemberModelAdmissionMode string `mapstructure:"enterprise_member_model_admission_mode"`
...
type OpsCleanupConfig struct {
	...
	// Error and metrics targets accept 0 as an explicit truncate;
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	trustedProxiesSetBeforeDefaults := viper.IsSet("server.trusted_proxies")
	...
	trustedProxiesConfigured := viper.InConfig("server.trusted_proxies") ||
		trustedProxiesSetBeforeDefaults || trustedProxiesEnvConfigured
	...
	if !trustedProxiesConfigured && len(cfg.Server.TrustedProxies) == 0 {
