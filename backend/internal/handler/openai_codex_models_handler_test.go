...
func performCodexModelsRequestForPlatform(t *testing.T, handler *OpenAIGatewayHandler, groupID int64, platform string) *httptest.ResponseRecorder {
	t.Helper()
	recorder, _ := performCodexModelsRequestWithContext(t, handler, groupID, platform)
	return recorder
}

func performCodexModelsRequestWithContext(t *testing.T, handler *OpenAIGatewayHandler, groupID int64, platforms ...string) (*httptest.ResponseRecorder, *gin.Context) {
	t.Helper()
	platform := service.PlatformOpenAI
	if len(platforms) > 0 {
