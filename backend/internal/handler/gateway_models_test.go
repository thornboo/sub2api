...
func TestGatewayCodexModels_CompositeUsesCompleteEffectiveModelList(t *testing.T) {
	...
	var got codexModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	want := service.FilterCodexModelIDsForGroup(defaultCodexModelIDsForPlatform(service.PlatformOpenAI), nil)
...
func TestGatewayModels_CompositeCustomModelsListFiltersAcrossConcretePlatforms(t *testing.T) {
			...
			byGroup: map[int64][]service.Account{
				groupID: {
					...
					},
					{
						...
						Credentials: map[string]any{
							"model_mapping": map[string]any{"opencode-custom": "grok-4.6"},
						},
					},
				},
			},
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelAllowlist: service.GroupModelAllowlist{
				Enabled: true,
				Models:  []string{"gemini-2.5-flash", "missing-model", "ag-custom-model", "gpt-5.5", "kimi-custom", "glm-custom", "deepseek-custom", "minimax-custom", "opencode-custom"},
			},
		},
	})
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"gemini-2.5-flash", "ag-custom-model", "gpt-5.5", "kimi-custom", "glm-custom", "deepseek-custom", "minimax-custom", "opencode-custom"}, modelIDsForTest(got.Data))
}
