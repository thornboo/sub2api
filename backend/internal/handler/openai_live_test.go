...
func TestLiveCallIdentityCarriesEnterpriseMemberSnapshot(t *testing.T) {
	gin.SetMode(gin.TestMode)
	request := httptest.NewRequest("POST", "/v1/live", nil)
	request.Header.Set("User-Agent", "live-client")
	context, _ := gin.CreateTestContext(httptest.NewRecorder())
	context.Request = request
	groupID := int64(44)
	memberID := int64(55)
	subscription := &service.UserSubscription{ID: 66}

	identity := liveCallIdentity(context, &service.APIKey{
		ID:       22,
		UserID:   33,
		GroupID:  &groupID,
		MemberID: &memberID,
	...
	require.Equal(t, int64(22), identity.APIKeyID)
	require.Equal(t, int64(33), identity.UserID)
	require.Equal(t, groupID, *identity.GroupID)
	require.Equal(t, memberID, *identity.MemberID)
	require.Equal(t, "member-55", identity.MemberCodeSnapshot)
	require.Equal(t, "Member 55", identity.MemberNameSnapshot)
	require.Equal(t, subscription.ID, *identity.SubscriptionID)
	require.Equal(t, "live-client", identity.UserAgent)
	require.Equal(t, "/v1/live", identity.InboundEndpoint)
}

func TestLiveEnabledForAPIKey(t *testing.T) {
	require.False(t, liveEnabledForAPIKey(nil, nil))
...
func TestLiveEnabledForCompositeResolvedOpenAI(t *testing.T) {
	...
	context.Request = context.Request.WithContext(service.WithResolvedTargetPlatform(context.Request.Context(), service.PlatformOpenAI))

	require.True(t, liveEnabledForAPIKey(context, &service.APIKey{
		Group: &service.Group{Platform: service.PlatformComposite, AllowLive: true},
	}))

	context.Request = context.Request.WithContext(service.WithResolvedTargetPlatform(context.Request.Context(), service.PlatformGrok))
	require.False(t, liveEnabledForAPIKey(context, &service.APIKey{
		Group: &service.Group{Platform: service.PlatformComposite, AllowLive: true},
	}))
}
