...
// 后续 turn 切换到白名单外的模型：连接模型锁定先拒绝，不能在已选
// WebSocket 会话上重路由。passthrough relay 模式返回 reconnect 提示。
func TestOpenAIResponsesWebSocket_SubsequentTurnModelNotAllowedCloses_Passthrough(t *testing.T) {
	runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
		...
		secondTurnCloseReasonContains: "model switch requires reconnect",
	})
}

// 后续 turn 切换到白名单外的模型：原生 ingress 也保持连接模型锁定，
// 不在已选 WebSocket 会话上重路由。
func TestOpenAIResponsesWebSocket_SubsequentTurnModelNotAllowedCloses_NativeIngress(t *testing.T) {
	runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
		...
		ingressMode:                   service.OpenAIWSIngressModeDedicated,
		secondTurnCloseExpected:       true,
		secondTurnCloseReasonContains: "model cannot change within an active websocket connection",
	})
}
...
// 白名单关闭：非白名单模型本身不被拦截；连接内模型切换仍由
// WebSocket 路由锁定规则独立处理。
