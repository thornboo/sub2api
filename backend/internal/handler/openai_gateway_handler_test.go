...
func TestOpenAIResponsesWebSocket_PassthroughTracksLockedModelPerTurn(t *testing.T) {
	got := runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
	...
	require.Equal(t, "sol", gjson.GetBytes(got.clientEvents[1], "response.model").String())

	require.Len(t, got.logs, 2)
	for _, usageLog := range got.logs {
		require.Equal(t, "sol", usageLog.Model)
		require.Equal(t, "sol", usageLog.RequestedModel)
		...
		require.Equal(t, "sol→sol-channel→gpt-5.6-sol", *usageLog.ModelMappingChain)
	}
	require.InDelta(t, got.logs[0].TotalCost, got.logs[1].TotalCost, 1e-12,
		"each turn must be billed with the route locked for the connection")
}
...
func TestOpenAIResponsesWebSocket_PassthroughKeepsSessionRouteWhenChannelMappingChanges(t *testing.T) {
	got := runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
		firstPayload:  `{"type":"response.create","model":"sol","stream":false}`,
		secondPayload: `{"type":"response.create","model":"sol","stream":false}`,
		channelMapping: map[string]string{
	...
	require.Equal(t, "sol→gpt-5.6-sol", *got.logs[1].ModelMappingChain)
	require.InDelta(t, got.logs[0].TotalCost, got.logs[1].TotalCost, 1e-12,
		"mapping changes must take effect only after reconnecting")
}

func TestOpenAIResponsesWebSocket_CtxPoolTracksLockedMappingAndPreservesRequestedModel(t *testing.T) {
	got := runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
	...
	require.Equal(t, "gpt-5.6-sol", gjson.GetBytes(got.clientEvents[1], "response.model").String())

	require.Len(t, got.logs, 2)
	for _, usageLog := range got.logs {
		require.Equal(t, "gpt-5.6-sol", usageLog.RequestedModel)
		require.NotNil(t, usageLog.ModelMappingChain)
		require.Equal(t, "gpt-5.6-sol→gpt-5.6-terra", *usageLog.ModelMappingChain)
		require.InDelta(t, 40e-6, usageLog.TotalCost, 1e-12,
			"BillingModelSourceRequested must use the public model before channel mapping")
	}
}
...
func TestShouldReportOpenAIWSProxyAccountFailure(t *testing.T) {
	...
	t.Run("session route lock rejection does not penalize account", func(t *testing.T) {
...
type openAIResponsesWSUsageLogCase struct {
	...
	// secondTurnCloseReasonContains 覆盖默认的白名单错误断言；用于模型锁定等
	// 更早拒绝路径。
	secondTurnCloseReasonContains string
}
...
func runOpenAIResponsesWebSocketUsageLogCase(t *testing.T, tc openAIResponsesWSUsageLogCase) openAIResponsesWSUsageLogResult {
	...
	if strings.TrimSpace(tc.secondPayload) != "" && (turnCount >= 2) {
		...
		if tc.secondTurnCloseExpected {
			...
			require.ErrorAs(t, readErr, &closeErr)
			require.Equal(t, coderws.StatusPolicyViolation, closeErr.Code)
			wantReason := tc.secondTurnCloseReasonContains
			if wantReason == "" {
				wantReason = "not available for this group"
			}
