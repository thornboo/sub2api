...
func (h *AccountHandler) respondAccountList(c *gin.Context, q accountListQuery, accounts []service.Account, total int64, withETag bool) {
	...
	if q.lite {
		...
		etag := buildAccountsListETag(compact, total, q.page, q.pageSize, q.platform, q.accountType, q.status, q.search, true)
		if etag != "" {
			c.Header("ETag", etag)
		...
		response.Paginated(c, compact, total, q.page, q.pageSize)
		return
	}

	etag := buildAccountsListETag(result, total, q.page, q.pageSize, q.platform, q.accountType, q.status, q.search, false)
	if etag != "" {
		c.Header("ETag", etag)
		c.Header("Vary", "If-None-Match")
		if ifNoneMatchMatched(c.GetHeader("If-None-Match"), etag) {
