...
func (w *opsCaptureWriter) inactiveWriteError() error {
	if w == nil || w.state == nil {
		return io.ErrClosedPipe
	}
	state := w.state
	state.mu.RLock()
	stale := state.generation != w.generation
	state.mu.RUnlock()
	if stale {
		return nil
	}
...
func OpsErrorLoggerMiddleware(ops *service.OpsService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer func() {
			...
			// released generation-bound handle, even if a downstream wrapper retained it.
...
func logOpsRecoveredUpstream(c *gin.Context, ops *service.OpsService, finalStatus int) {
	...
	for i := len(entry.UpstreamErrors) - 1; i >= 0; i-- {
		if event := entry.UpstreamErrors[i]; event != nil {
			...
			if platform := strings.TrimSpace(event.Platform); platform != "" {
	...
	applyOpsFailureClassification(entry, classifyOpsFailureV2(
		c,
		entry.ErrorType,
		entry.ErrorMessage,
		"",
	...
	if apiKey != nil {
		...
		if strings.TrimSpace(entry.Platform) == "" && apiKey.Group != nil && apiKey.Group.Platform != "" {
