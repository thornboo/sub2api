...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		for {
			...
			account := selection.Account
			// SelectAccountWithLoadAwareness may register an Anthropic OAuth/setup-token
			// session before returning a WaitPlan or acquired slot. Track it before
			// any intercept, queue, wait, or profit early return so failure paths can
			// release the session immediately instead of waiting for idle expiry.
...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if platform == service.PlatformComposite {
		...
		if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
			source := modelListingSource(service.PlatformComposite, availableModels, defaultModelIDsForPlatform(service.PlatformComposite))
			...
			writeAllowlistedModelsList(c, service.PlatformComposite, availableModels, endpointTypes)
			return
		}
	...
	if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
		...
		endpointTypes := h.supportedEndpointTypesForGroups(c.Request.Context(), groupIDs, availableModels)
		writeAllowlistedModelsList(c, platform, availableModels, endpointTypes)
		return
	}
...
func (h *GatewayHandler) codexModelIDsForGroup(ctx context.Context, group *service.Group, platformOverride string) []string {
	...
	if platform == service.PlatformComposite {
		...
		// Composite Codex manifests must reflect the concrete candidate pools, including
		// defaults contributed by unmapped OpenAI-compatible accounts when mapped
		// accounts also exist. configuredCompositeModels intentionally uses only
		// persistent mapping metadata, so supplement from the effective per-platform
		// discovery projection here.
		availableModels = appendEffectivePlatformModels(availableModels, h.gatewayService.GetAvailableModels(ctx, groupID, service.PlatformOpenAI))
		availableModels = appendEffectivePlatformModels(availableModels, h.gatewayService.GetAvailableModels(ctx, groupID, service.PlatformGrok))
...
func appendEffectivePlatformModels(models []string, candidates []string) []string {
	...
	return appendNormalizedModels(models, seen, candidates)
}
...
func writeAllowlistedModelsList(c *gin.Context, platform string, modelIDs []string, endpointTypes map[string][]string) {
	if platform == service.PlatformOpenAI {
		writeOpenAIModelsList(c, modelIDs, endpointTypes)
		return
	}
...
func (h *GatewayHandler) Usage(c *gin.Context) {
	...
	if h.usageService != nil {
		if stats, err := h.usageService.GetAPIKeyModelStats(ctx, subject.UserID, apiKey.ID, startTime, endTime); err == nil && len(stats) > 0 {
			modelStats = stats
		}
	}
