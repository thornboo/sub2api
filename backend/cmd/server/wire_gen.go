...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	enterpriseMemberRepository := repository.NewEnterpriseMemberRepository(client, db)
	enterpriseMemberAuditRepository := repository.NewEnterpriseMemberAuditRepository(db)
	enterpriseMemberService := service.NewEnterpriseMemberService(enterpriseMemberRepository, userRepository, apiKeyService, enterpriseMemberAuditRepository)
	enterpriseMemberBudgetRepository := repository.NewEnterpriseMemberBudgetRepository(db)
	...
	channelService := service.NewChannelService(channelRepository, groupRepository, apiKeyAuthCacheInvalidator, pricingService, billingService, channelCachePubSub)
	...
	enterpriseMemberImportService := service.NewEnterpriseMemberImportService(enterpriseMemberImportRepository, secretEncryptor, apiKeyService)
	enterpriseMemberHandler := handler.NewEnterpriseMemberHandler(enterpriseMemberService, enterpriseMemberBudgetService, enterpriseMemberImportService, enterpriseMemberAuditRepository)
	...
	modelFailCounterCache := repository.NewModelFailCounterCache(redisClient)
	geminiTokenCache := repository.NewGeminiTokenCache(redisClient)
	compositeTokenCacheInvalidator := service.NewCompositeTokenCacheInvalidator(geminiTokenCache)
	rateLimitService := service.ProvideRateLimitService(accountRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, timeoutCounterCache, openAI403CounterCache, modelFailCounterCache, settingService, compositeTokenCacheInvalidator)
	...
	modelProtocolCapabilityRepository := repository.NewModelProtocolCapabilityRepository(db)
	modelProtocolCapabilityService := service.ProvideModelProtocolCapabilityService(modelProtocolCapabilityRepository, accountRepository, groupRepository, channelService, configConfig, settingService)
	openAIGatewayService := service.ProvideOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, serviceUserPlatformQuotaRepository, modelProtocolCapabilityService)
	...
	announcementKeyReadRepository := repository.NewAnnouncementKeyReadRepository(db)
	announcementService := service.NewAnnouncementService(announcementRepository, announcementReadRepository, announcementKeyReadRepository, userRepository, userSubscriptionRepository)
	announcementHandler := handler.NewAnnouncementHandler(announcementService)
	feedbackRepository := repository.NewFeedbackRepository(db)
	feedbackCooldownStore := repository.NewFeedbackCooldownStore(redisClient)
	feedbackService := service.NewFeedbackService(feedbackRepository, feedbackCooldownStore)
	feedbackHandler := handler.NewFeedbackHandler(feedbackService, apiKeyService)
	...
	modelDeliveryService := service.ProvideModelDeliveryService(accountRepository, groupRepository, channelService, modelProtocolCapabilityService, compositeRouteResolver, configConfig, settingService)
	accountHandler := admin.ProvideAccountHandler(configConfig, adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, modelProtocolCapabilityService, modelDeliveryService)
	...
	enterpriseMemberAliasReviewRepository := repository.NewEnterpriseMemberAliasReviewRepository(db)
	enterpriseMemberAliasReviewService := service.ProvideEnterpriseMemberAliasReviewService(enterpriseMemberAliasReviewRepository, channelService, modelDeliveryService, groupRepository)
	opsHandler := admin.ProvideOpsHandler(opsService, enterpriseMemberBudgetService, enterpriseMemberAliasReviewService)
	...
	channelHandler := admin.NewChannelHandler(channelService, billingService, pricingService, modelDeliveryService)
	...
	modelSelfCheckHandler := admin.NewModelSelfCheckHandler(modelSelfCheckService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, adminFeedbackHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, modelSelfCheckHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	modelRuntimeRepository := repository.NewModelRuntimeRepository(db)
	modelRuntimeService := service.ProvideModelRuntimeService(modelRuntimeRepository, opsService)
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService, modelDeliveryService, modelRuntimeService)
	modelPlazaHandler := handler.NewModelPlazaHandler(channelService, settingService, modelDeliveryService, modelRuntimeService)
	imageTaskStore := repository.NewImageTaskStore(redisClient)
	imageTaskService := service.ProvideImageTaskService(imageTaskStore, imageStorageSettingService, enterpriseMemberBudgetService)
	...
	enterpriseMemberUsageSettlementRepository := repository.NewEnterpriseMemberUsageSettlementRepository(client, db)
	enterpriseMemberBudgetRecoveryService := service.ProvideEnterpriseMemberBudgetRecoveryService(enterpriseMemberBudgetRepository, enterpriseMemberUsageSettlementRepository)
	enterpriseMemberImportCleanupService := service.ProvideEnterpriseMemberImportCleanupService(enterpriseMemberImportRepository)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, enterpriseMemberHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, feedbackHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService, enterpriseMemberBudgetRecoveryService, enterpriseMemberImportCleanupService, openAIQuotaAutoResetService)
	...
	routingEligibilityRevisionRepository := repository.NewRoutingEligibilityRevisionRepository(db)
	routingEligibilityEventBus := repository.NewRoutingEligibilityEventBus(redisClient)
	routingEligibilityRuntime := service.ProvideRoutingEligibilityRuntime(routingEligibilityRevisionRepository, routingEligibilityEventBus, channelService, modelProtocolCapabilityService)
	enterpriseMemberAdmissionEvidenceRepository := repository.NewEnterpriseMemberAdmissionEvidenceRepository(db)
	enterpriseMemberAdmissionEvidenceService := service.NewEnterpriseMemberAdmissionEvidenceService(enterpriseMemberAdmissionEvidenceRepository)
	enterpriseMemberModelAdmissionReadinessProvider := service.ProvideEnterpriseMemberModelAdmissionReadinessProvider(routingEligibilityRuntime, enterpriseMemberAliasReviewService, enterpriseMemberAdmissionEvidenceService)
	enterpriseMemberRoutePlanner := service.ProvideEnterpriseMemberRoutePlanner(channelService, modelDeliveryService, routingEligibilityRuntime, enterpriseMemberModelAdmissionReadinessProvider)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, enterpriseMemberBudgetService, subscriptionService, opsService, settingService, compositeRouteResolver, enterpriseMemberRoutePlanner, redisClient)
	...
	enterpriseMemberImportWorker := service.ProvideEnterpriseMemberImportWorker(enterpriseMemberImportRepository, enterpriseMemberImportService)
	...
	modelSelfCheckRunner := service.ProvideModelSelfCheckRunner(modelSelfCheckService, settingService)
	channelMonitorV2Aggregator := service.ProvideChannelMonitorV2Aggregator(channelMonitorV2Repository, db, settingService)
	userPlatformQuotaUsageFlusher := service.ProvideUserPlatformQuotaUsageFlusher(configConfig, billingCache, serviceUserPlatformQuotaRepository, timingWheelService)
	upstreamSupplierBalancePoller := service.ProvideUpstreamSupplierBalancePoller(client, configConfig, secretEncryptor)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, schedulerSnapshotService, routingEligibilityRuntime, tokenRefreshService, accountExpiryService, cnProviderBalanceCheckService, openAICodexVersionSyncService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, enterpriseMemberImportWorker, batchImageCleanupService, batchImageWorkerRuntime, imageTaskService, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, modelSelfCheckRunner, channelMonitorV2Aggregator, userPlatformQuotaUsageFlusher, upstreamBillingProbeService, upstreamSupplierBalancePoller, ollamaCloudUsageService, auditLogService, openAIQuotaAutoResetService, promptService, pluginManager)
