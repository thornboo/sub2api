...
type GroupQuery struct {
	...
	withEnterpriseMembers             *EnterpriseMemberQuery
...
// QueryEnterpriseMembers chains the current query on the "enterprise_members" edge.
func (_q *GroupQuery) QueryEnterpriseMembers() *EnterpriseMemberQuery {
...
func (_q *GroupQuery) QueryEnterpriseMemberGroupBindings() *EnterpriseMemberGroupBindingQuery {
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withEnterpriseMembers:             _q.withEnterpriseMembers.Clone(),
...
// WithEnterpriseMembers tells the query-builder to eager-load the nodes that are connected to
// the "enterprise_members" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithEnterpriseMembers(opts ...func(*EnterpriseMemberQuery)) *GroupQuery {
	query := (&EnterpriseMemberClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withEnterpriseMembers = query
	return _q
}
...
func (_q *GroupQuery) WithEnterpriseMemberGroupBindings(opts ...func(*EnterpriseMemberGroupBindingQuery)) *GroupQuery {
	query := (&EnterpriseMemberGroupBindingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withEnterpriseMembers; query != nil {
		if err := _q.loadEnterpriseMembers(ctx, query, nodes,
			func(n *Group) { n.Edges.EnterpriseMembers = []*EnterpriseMember{} },
			func(n *Group, e *EnterpriseMember) { n.Edges.EnterpriseMembers = append(n.Edges.EnterpriseMembers, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withEnterpriseMemberGroupBindings; query != nil {
		if err := _q.loadEnterpriseMemberGroupBindings(ctx, query, nodes,
			func(n *Group) { n.Edges.EnterpriseMemberGroupBindings = []*EnterpriseMemberGroupBinding{} },
			func(n *Group, e *EnterpriseMemberGroupBinding) {
...
func (_q *GroupQuery) loadEnterpriseMembers(ctx context.Context, query *EnterpriseMemberQuery, nodes []*Group, init func(*Group), assign func(*Group, *EnterpriseMember)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Group)
	nids := make(map[int64]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.EnterpriseMembersTable)
		s.Join(joinT).On(s.C(enterprisemember.FieldID), joinT.C(group.EnterpriseMembersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.EnterpriseMembersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.EnterpriseMembersPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*EnterpriseMember](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "enterprise_members" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *GroupQuery) loadEnterpriseMemberGroupBindings(ctx context.Context, query *EnterpriseMemberGroupBindingQuery, nodes []*Group, init func(*Group), assign func(*Group, *EnterpriseMemberGroupBinding)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(enterprisemembergroupbinding.FieldGroupID)
	}
	query.Where(predicate.EnterpriseMemberGroupBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.EnterpriseMemberGroupBindingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
