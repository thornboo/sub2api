export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      keyPool: {
        title: '上游 API Key 池',
        hint: '同一上游账号内按优先级和最近使用时间轮询；失败时只冷却当前 Key + 当前上游模型。',
        add: '添加 Key',
        empty: '请添加至少一个上游 API Key。',
        required: '请在上游 API Key 池中至少添加一个 Key',
        rowRequired: '每个上游 API Key 都需要同时填写备注和 Key',
        modelsRequired: '每个上游 API Key 都需要配置自己的模型白名单或模型映射',
        noteLabel: '备注',
        apiKeyLabel: 'API Key',
        priorityLabel: '优先级',
        priorityHint: '数值越小优先级越高；同优先级按最近使用时间轮询。',
        statusLabel: '状态',
        namePlaceholder: '例如：packycode 分组 A',
        apiKeyPlaceholder: '填写上游 API Key',
        apiKeyPlaceholderEdit: '留空保持原 Key 不变',
        statusActive: '启用',
        statusInactive: '停用',
        cooling: '冷却中',
        requests: '请求 {count}',
        ...
        modelRestrictionTitle: '该 Key 的模型限制',
        modelRestrictionHint: '每个 Key 独立配置模型能力；调度只会把请求发给匹配当前模型的 Key。',
        whitelistPlaceholder: '每行一个该 Key 支持的上游模型，例如：\nclaude-haiku-4.5'
      },
      poolMode: '池模式',
