...
export default {
    ...
    channelStatus: {
        ...
        selfCheckChain: {
            title: '探测链路',
            description: '按优先级依次探测，首个成功即结束。健康探测结果可能不同于实际请求路由。',
            retry: '重试',
            loadError: '加载探测链路失败',
            empty: '暂无探测链路数据',
            currentCandidates: '可参与账号',
            lastProbe: '上次探测',
            roundStatus: '最近轮次',
            roundTimeout: '轮次超时',
            attemptTimeout: '单账号超时',
            currentOrder: '当前候选顺序',
            latestRound: '最近执行链路',
            updatedAt: '配置视图更新：{time}',
            noCandidates: '当前没有可展示的候选账号。',
            noRound: '暂无轮次',
            noRoundDetail: '还没有持久化的真实探测轮次。',
            noSteps: '该轮次没有步骤记录。',
            noLatency: '无延迟',
            neverChecked: '未检测',
            eligible: '可参与本轮探测',
            ...
            reason: {
                ...
                no_eligible_account: '无符合条件账号',
                model_not_supported: '不支持该模型',
                account_inactive: '账号未启用',
                account_not_schedulable: '账号不可调度',
                disabled: '账号已禁用',
                not_schedulable: '账号不可调度',
                group_removed: '账号已移出当前分组',
                expired: '账号已过期',
                overloaded: '账号负载过高',
                rate_limited: '账号限流中',
                temporarily_unschedulable: '账号临时不可调度',
                quota_exceeded: '账号额度不足',
                model_cooldown: '模型冷却中',
                temp_banned: '临时封禁',
                model_rate_limited: '模型限流中',
                parent_unhealthy: '父账号不可用',
                parent_unavailable: '父账号不可用',
                platform_mismatch: '平台不匹配',
                prior_success: '前序账号已成功',
                round_timeout: '轮次超时',
                round_deadline: '轮次达到截止时间',
                attempt_timeout: '单账号超时',
                cancelled: '任务已取消',
                probe_failed: '探测失败',
    ...
    availableChannels: {
        ...
        export: {
            ...
            columns: {
                ...
                billingMode: '计费模式',
                interval: '区间',
                inputPrice: '输入生效价（$/1M token）',
                outputPrice: '输出生效价（$/1M token）',
                cacheWritePrice: '缓存写入生效价（$/1M token）',
                cacheReadPrice: '缓存读取生效价（$/1M token）',
                imageOutputPrice: '图片输出生效价（$/次）',
                perRequestPrice: '按次生效价（$/次）',
                intervals: '阶梯定价'
            }
        },
        pricing: {
    ...
    admin: {
        ...
        accounts: {
            ...
            upstreamCost: {
                ...
                officialPricingChannel: '上游官方价目表',
                officialPricingChannelPlaceholder: '请选择维护官方价格的渠道',
                officialPricingChannelRequired: '人民币计价必须选择明确的上游官方价目表',
                officialPricingChannelHint: '这里仅把所选渠道当作官方价目表容器；成本计算不会再读取调用用户所在分组的价格。美元价可留空并使用内置官方价。',
                ...
                supplierBalance: {
                    title: '供应商账户余额',
                    description: '查询供应商上游登录账户的钱包余额，不统计单个 Key 配额。',
                    enable: '启用查询',
                    provider: '站点类型',
                    ...
                    accessTokenPlaceholder: '填写 New API 个人 Access Token',
                    accessTokenConfigured: '已配置，留空则保留',
                    accessTokenHint: 'Token 只用于查询供应商账户余额，不参与模型请求转发。',
                    apiKey: 'API Key',
                    apiKeyPlaceholder: '填写 SoleAPI API Key，例如 sk-sole-...',
                    apiKeyHint: 'API Key 只用于查询 SoleAPI 账户余额，不参与模型请求转发。',
                    sub2ApiKeyPlaceholder: '填写上游 sub2api 站点的 API Key',
                    sub2ApiKeyHint: '使用余额计费分组、未设置总额度或周期额度限制的 API Key；仅查询账号钱包余额，不统计 Key 或订阅额度。',
                    accessTokenProviderChangedHint: '站点类型已变化，请重新填写凭证。',
                    accessTokenSiteChangedHint: '站点已变化，请重新填写凭证。',
                    balance: '账户余额',
                    disabled: '未启用',
                    disabledHint: '未配置账户余额查询',
                    pending: '未查询',
                    unknown: '未知',
                    detailsNotLoaded: '供应商详情未加载',
                    failed: '查询失败',
                    notFetched: '还没有刷新过',
                    updatedAt: '更新于 {time}',
