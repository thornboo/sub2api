export default {
    settings: {
      ...
      features: {
        ...
        availableChannels: {
          title: '模型列表',
          description: '向已登录用户展示其可访问分组中的模型、价格与运行表现。',
          configureLink: '前往 渠道管理 > 渠道定价 配置模型价格',
          enabled: '启用模型列表',
          enabledHint: '关闭后，用户端将隐藏入口，不再展示模型列表。',
        },
        siteBillingMode: {
          title: '站点类型',
        ...
        modelPlaza: {
          ...
          enabled: '启用公开模型列表',
          enabledHint: '开启后 Home 首页显示入口，页面可通过 /model-plaza 独立访问。',
          requireAuth: '需要登录才可访问',
          requireAuthHint: '开启后未登录访问将跳转登录页；关闭则公开可见，匿名访客仅展示非专属分组。',
          priceDescription: '价格说明（Markdown）',
          priceDescriptionHint: '展示在公开模型列表页面顶部，可用于说明计费规则、汇率、优惠活动等。',
        },
        pluginManagement: {
          title: '插件管理',
      ...
      gatewayForwarding: {
        ...
        nativeModelProtocolRouting: '模型多 API 端点路由',
        nativeModelProtocolRoutingHint: '开启后，账号中已确认支持的模型协议会同时参与用户模型列表的 API 端点展示和真实请求路由。关闭不会删除能力配置，只会保持原有兼容路径。',
        routingEnabled: '已启用',
        routingDisabled: '未启用',
        sourceSettings: '来源：后台设置',
        sourceConfig: '来源：配置文件默认值',
        sourceSettingsInvalid: '来源：后台设置无效值',
        sourceConfigInvalid: '来源：配置文件无效默认值',
        sourceErrorFallback: '来源：安全降级默认值',
        sourceEnforceBlocked: '来源：强制模式未就绪，已安全降级',
        enableRoutingConfirmTitle: '启用模型多 API 端点路由？',
        enableRoutingConfirmMessage: '启用后，已确认支持的 API 端点会进入用户模型目录，并可能优先使用原生上游协议处理新请求。请确认账号协议能力配置准确；进行中的请求不受影响。',
        ...
        enterpriseMemberAdmissionEnforceHint: '只有具备精确发布模型和稳定投递候选的分组才会执行请求。',
        enterpriseMemberAdmissionSourceWarning: '当前来源无效或处于降级状态；启用强制模式前请核对已保存设置或部署配置。',
        enterpriseMemberAdmissionEnforceConfirmTitle: '强制企业成员已发布模型准入？',
        enterpriseMemberAdmissionEnforceConfirmMessage: '企业成员请求将不再回退到第一个授权分组。只有具备精确发布资格和稳定投递资格的分组才能执行该模型。',
        enterpriseMemberAdmissionEnforceConfirmAction: '确认强制准入',
        enterpriseMemberAdmissionEnforceBlocked: '阶段 3 前置条件尚未完成，服务端已禁用全局强制准入。当前只能使用影子计算或旧顺序回滚模式。',
        enterpriseMemberAdmissionReadiness: '就绪条件',
        enterpriseMemberAdmissionReady: '可启用强制准入',
        enterpriseMemberAdmissionNotReady: '尚未就绪',
        enterpriseMemberAdmissionRollout: '灰度范围',
        enterpriseMemberAdmissionRolloutValue: '{source} · {percentage}',
        enterpriseMemberAdmissionRolloutDetail: '{enterprises} 个企业用户，{members} 个成员，hash bucket {bucket}，{matched}',
        enterpriseMemberAdmissionAutoStop: '自动停止',
        enterpriseMemberAdmissionAutoStopped: '已由护栏停止',
        enterpriseMemberAdmissionAutoStopArmed: '护栏已启用',
        enterpriseMemberAdmissionAutoStopOff: '未配置',
        enterpriseMemberAdmissionLegacyDeprecation: '旧顺序退役',
        enterpriseMemberAdmissionLegacyUsage: '当前进程生效次数：{count}',
        enterpriseMemberAdmissionLegacyRetirementValue: '{status} · {target}',
        enterpriseMemberAdmissionRetirementNotScheduled: '未排期',
        enterpriseMemberAdmissionLegacyRetirementTarget: '退役目标',
        enterpriseMemberAdmissionLegacyRetirementPlaceholder: '2026-09-30 或 v1.8.0',
        enterpriseMemberAdmissionLegacyRetirementHint: '必须显式填写 ISO 日期或版本；未填写时 Phase 5 仍为 not_scheduled。',
        enterpriseMemberAdmissionRiskReductionOnly: 'legacy_order_only 已废弃，仅允许作为缩小生产风险的紧急回滚，不得用于扩大路由行为。',
        enterpriseMemberAdmissionEnterpriseIds: '企业用户 ID',
        enterpriseMemberAdmissionMemberIds: '成员 ID',
        enterpriseMemberAdmissionIdsPlaceholder: '例如：12, 13, 21',
        enterpriseMemberAdmissionPercentage: '灰度百分比',
        enterpriseMemberAdmissionSalt: '稳定 hash salt',
        enterpriseMemberAdmissionManualStop: '手动紧急停止 rollout',
        enterpriseMemberAdmissionAliasReview: 'Alias 审核',
        enterpriseMemberAdmissionAliasReviewHint: '强制准入前，审核最近 7 天和 30 天观测到的精确 alias 候选。',
        enterpriseMemberAdmissionAliasEmpty: '暂无需要审核的 alias 候选。',
        enterpriseMemberAdmissionAliasLoadFailed: '加载 alias 审核清单失败',
        enterpriseMemberAdmissionAliasReviewed: 'Alias 审核已保存',
        enterpriseMemberAdmissionAliasReviewFailed: '保存 alias 审核失败',
        aliasModel: 'Alias',
        aliasTarget: '目标',
        aliasUsage7d: '7天',
        aliasUsage30d: '30天',
      ...
      openaiFastPolicy: {
        ...
        removeUser: '移除用户',
        userIdsValidationError: '用户 ID 必须是大于 0 的整数，且同一规则内不能重复。',
        errorMessage: '错误消息',
        errorMessagePlaceholder: '拦截时返回的自定义错误消息',
        errorMessageHint: '留空则使用默认错误消息。',
        modelWhitelist: '目标模型',
