export default {
    ...
    users: {
      ...
      form: {
        ...
        roleLabel: '角色',
		accountType: '账户类型',
		accountTypes: { individual: '个人账户', enterprise: '企业账户' },
		accountTypeHint: '企业能力与登录角色分离；管理员角色不能设为企业账户。',
		enterpriseCapability: '启用企业成员能力',
		enterpriseCapabilityHint: '关闭后全部成员密钥立即失效，但成员、预算、密钥和用量历史都会保留。',
        selectRole: '选择角色',
        balanceLabel: '余额',
      ...
      balanceConsumption: {
        ...
        frozenBalance: '另有 {amount} 已冻结',
        todayConsumption: '今日消费',
        actualDeduction: '实际钱包扣款',
        periodConsumption: '近 {days} 天消费',
        dailyAverage: '日均消费',
        calendarDayAverage: '按自然日计算',
        forecastTitle: '余额预测',
        forecastDays: '预计还可使用约 {days} 天',
        forecastLessThanDay: '预计不足 1 天',
        forecastDepleted: '余额已耗尽',
        forecastNoConsumption: '暂无预计耗尽时间',
        forecastCalculating: '正在计算预测…',
        forecastDate: '按近 {days} 天趋势，约 {date} 耗尽',
        requestCount: '{count} 次请求',
        chartTitle: '每日余额消费',
        chartHint: '只统计钱包余额计费，订阅额度消费不会计入',
        rangeLabel: '消费趋势时间范围',
        rangeDays: '{days} 天',
        dailySpend: '余额消费',
        noData: '所选时间范围内暂无余额消费',
        loadFailed: '加载余额消费趋势失败'
      },
      // Settings Dropdowns
      filterSettings: '筛选设置',
      columnSettings: '列设置',
    ...
    groups: {
      ...
      sortOrder: '展示排序',
      columnSettings: '列设置',
      sortOrderHint: '拖拽分组后点击保存，统一调整分组选择与模型列表的展示顺序。停用分组保留排序位置；此设置不影响账号调度。',
      sortOrderValueHint: '数值越小越靠前。通过「展示排序」拖拽调整并保存。',
      ...
      openaiMessages: {
        ...
        familyMappingTitle: 'Claude 模型名处理',
        familyMappingHint: '明确选择保留客户端模型名，或将 Claude 系列确定性映射到当前分组的逻辑模型。',
        passthroughTitle: '原样透传',
        passthroughHint: '不做 Opus、Sonnet、Haiku 系列改写。',
        passthroughActiveHint: '未命中精确模型覆盖时，将保留客户端提交的原始模型名并继续账号模型匹配。',
        customTitle: '映射到分组模型',
        customHint: '按 Claude 系列改写为当前分组账号可承接的逻辑模型。',
        applyOpenAIPreset: '套用 OpenAI/Codex 预设',
        optionalModelPlaceholder: '留空则原样透传',
        createCandidateModelsHint: '创建分组时可手动填写；绑定账号后会提供分组模型候选。',
        candidateModelsHint: '候选来自当前分组可调度账号的模型映射左侧。',
        noCandidateModelsHint: '当前分组没有可枚举的账号模型候选，仍可手动填写逻辑模型名。',
        unlistedCandidateWarning: '该模型不在当前分组可枚举的账号候选中，请确认账号模型映射。',
        legacyMappingTitle: '当前沿用旧版 OpenAI/Codex 默认映射',
        legacyMappingHint: '为避免改变存量流量，旧分组继续使用原有 GPT 映射。请选择原样透传或转为显式自定义后，才会启用新语义。',
        convertToPassthrough: '改为原样透传',
        convertToCustom: '转为显式自定义',
        opusModel: 'Opus 映射模型',
