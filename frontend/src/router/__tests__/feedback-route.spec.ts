...
describe('feedback route integration', () => {
  it('exposes admin feedback management and signed-in user feedback entry', () => {
    ...
    expect(sidebarSource).toContain("{ path: '/admin/feedback', label: t('admin.feedback.title'), icon: TicketIcon }")
    expect(sidebarSource).toContain("{ path: '/feedback', label: t('feedback.myTickets'), icon: TicketIcon }")
    expect(appHeaderSource).not.toContain('to="/feedback"')
  })
})
