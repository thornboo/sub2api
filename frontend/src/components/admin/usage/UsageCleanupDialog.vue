<template>
  <BaseDialog :show="show" :title="t('admin.usage.cleanup.title')" width="wide" @close="handleClose">
    <div class="space-y-4">
      ...
      <div class="rounded-xl border border-stone-200/80 p-4 dark:border-white/10">
        ...
        <div class="mt-3 space-y-2">
          ...
          <div v-else class="space-y-2">
            <div
              v-for="task in tasks"
              :key="task.id"
              class="flex flex-col gap-2 rounded-lg border border-stone-200/70 px-3 py-2 text-sm text-gray-600 dark:border-white/10 dark:text-gray-300"
            >
...
const statusClass = (status: string) => {
  const map: Record<string, string> = {
    ...
    running: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-200',
    succeeded: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-200',
    failed: 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-200',
    canceled: 'bg-gray-200 text-gray-600 dark:bg-white/[0.08] dark:text-gray-300'
  }
