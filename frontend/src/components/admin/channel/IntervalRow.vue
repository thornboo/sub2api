<template>
  <div class="flex items-start gap-2 rounded border p-2"
    ...
    <template v-if="mode === 'token'">
      <div class="w-20">
        <label class="text-xs text-stone-400">Min</label>
      ...
      <div class="w-20">
        <label class="text-xs text-stone-400">Max <span class="text-stone-300 dark:text-stone-500">(含)</span></label>
      ...
      <div class="flex-1">
        <label class="text-xs text-stone-400">{{ t('admin.channels.form.inputPrice', '输入') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-stone-300 dark:text-stone-500">$/M</span></label>
      ...
      <div class="flex-1">
        <label class="text-xs text-stone-400">{{ t('admin.channels.form.outputPrice', '输出') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-stone-300 dark:text-stone-500">$/M</span></label>
        <input :value="interval.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-stone-400">{{ t('admin.channels.form.cacheWritePrice', '缓存W') }} <span class="text-stone-300 dark:text-stone-500">$/M</span></label>
        <input :value="interval.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-stone-400">{{ t('admin.channels.form.cacheReadPrice', '缓存R') }} <span class="text-stone-300 dark:text-stone-500">$/M</span></label>
    ...
    <template v-else>
      ...
      <div class="flex-1">
        <label class="text-xs text-stone-400">{{ t('admin.channels.form.perRequestPrice', '单次价格') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-stone-300 dark:text-stone-500">$</span></label>
