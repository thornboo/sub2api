<template>
  <div v-if="!isDesktopViewport" class="space-y-3">
    <template v-if="loading">
      <div v-for="i in 5" :key="i" class="rounded-lg border border-stone-200/80 bg-white/80 p-4 dark:border-white/10 dark:bg-neutral-950/70">
        <div class="space-y-3">
          <div v-for="column in dataColumns" :key="column.key" class="flex justify-between">
            <div class="h-4 w-20 animate-pulse rounded bg-stone-200 dark:bg-white/10"></div>
            <div class="h-4 w-32 animate-pulse rounded bg-stone-200 dark:bg-white/10"></div>
          </div>
          <div v-if="hasActionsColumn" class="border-t border-stone-200/80 pt-3 dark:border-white/10">
            <div class="h-8 w-full animate-pulse rounded bg-stone-200 dark:bg-white/10"></div>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="rounded-lg border border-stone-200/80 bg-white/80 p-12 text-center dark:border-white/10 dark:bg-neutral-950/70">
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="mb-4 h-12 w-12 text-stone-400 dark:text-stone-500"
            />
            <p class="text-lg font-medium text-stone-950 dark:text-stone-100">
              {{ t('empty.noData') }}
            </p>
          </div>
        </slot>
    ...
    <template v-else>
      ...
      <div
        ...
        :class="{
          ...
          'border-emerald-300 bg-emerald-50/40 dark:border-emerald-700 dark:bg-emerald-900/10': selectable && isRowSelected(row, index)
        }"
  ...
  >
    <table class="w-full min-w-max divide-y divide-stone-200/80 dark:divide-white/10">
          ...
          >
            <div :class="['flex items-center space-x-1', getHeaderContentAlignmentClass(column)]">
              ...
              <span
                ...
                class="inline-flex h-5 w-4 flex-col items-center justify-center text-stone-400 dark:text-stone-500"
                aria-hidden="true"
              >
                <svg
                  class="h-2.5 w-2.5"
      ...
      <tbody class="table-body divide-y divide-stone-200/70 bg-white/60 dark:divide-white/10 dark:bg-black/20">
        <!-- Loading skeleton -->
        <tr v-if="loading" v-for="i in 5" :key="i">
          ...
          <td v-for="column in columns" :key="column.key" :class="['whitespace-nowrap py-4', getAdaptivePaddingClass()]">
            <div class="animate-pulse">
              <div class="h-4 w-3/4 rounded bg-stone-200 dark:bg-white/10"></div>
            </div>
          </td>
        </tr>
        ...
        <tr v-else-if="!data || data.length === 0">
          <td
            ...
            :class="['py-12 text-center text-stone-500 dark:text-stone-400', getAdaptivePaddingClass()]"
          >
            <slot name="empty">
              <div class="flex flex-col items-center">
                <Icon
        ...
        <template v-else>
          ...
          <tr
            ...
            :class="{
              'cursor-pointer': clickableRows,
              'bg-emerald-50/40 dark:bg-emerald-900/10': selectable && isRowSelected(item.row, item.index)
            }"
...
.dark tbody tr:hover .sticky-col {
  background-color: rgb(15 15 15 / 0.98);
}
...
.table-wrapper::-webkit-scrollbar-track {
  background-color: rgba(120, 113, 108, 0.08) !important;
...
.table-wrapper::-webkit-scrollbar-thumb:hover {
  background-color: rgba(87, 83, 78, 0.68) !important;
}

.dark .table-wrapper::-webkit-scrollbar-thumb {
  background-color: rgba(168, 162, 158, 0.38) !important;
}
.dark .table-wrapper::-webkit-scrollbar-thumb:hover {
  background-color: rgba(214, 211, 209, 0.56) !important;
}

@supports (-moz-appearance:none) {
  .table-wrapper {
    scrollbar-width: thin !important;
    scrollbar-color: rgba(120, 113, 108, 0.5) rgba(0, 0, 0, 0.03) !important;
  }
  .dark .table-wrapper {
    scrollbar-color: rgba(168, 162, 158, 0.42) rgba(255, 255, 255, 0.05) !important;
  }
}
</style>
