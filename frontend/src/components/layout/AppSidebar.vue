...
const flagEnterpriseAccount = () => authStore.user?.role === 'user' && authStore.user?.account_type === 'enterprise'
const flagEnterpriseMembers = () => flagEnterpriseAccount() && !authStore.user?.enterprise_disabled_at

// buildSelfNavItems 构造用户自己的导航项（用户端主菜单和管理员的"我的账户"子菜单共享这组声明）。
// withDashboard=true 时包含仪表盘（用户端），false 时不含（管理员的个人区已经有独立仪表盘入口）。
//
// 条目顺序：密钥 → 用量 → 可用渠道 → 渠道状态 → 订阅/支付 → 兑换/资料/工单。
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/feedback', label: t('feedback.myTickets'), icon: TicketIcon },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/feedback', label: t('admin.feedback.title'), icon: TicketIcon },
