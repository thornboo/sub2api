<template>
  ...
  >
    ...
    >
            ...
            <div class="mb-3 flex flex-wrap gap-2">
              ...
              <div class="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  :key="'default-preset-' + preset.label"
                  type="button"
        ...
        <div class="border-t border-stone-200/80 pt-4 dark:border-white/10">
          ...
          <div v-else>
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <button
                v-for="preset in presetMappings"
                :key="'upstream-preset-' + preset.label"
                type="button"
                @click="addPresetMapping(preset.from, preset.to)"
                :class="['rounded-lg px-3 py-1 text-xs transition-colors', preset.color]"
              >
        ...
        <div class="border-t border-stone-200/80 pt-4 dark:border-white/10">
          ...
          <div v-else class="space-y-3">
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <button
                v-for="preset in presetMappings"
                :key="'bedrock-preset-' + preset.label"
                type="button"
                @click="addPresetMapping(preset.from, preset.to)"
                :class="['rounded-lg px-3 py-1 text-xs transition-colors', preset.color]"
              >
      ...
      <div
        v-if="account?.platform === 'openai' && (account?.type === 'oauth' || account?.type === 'setup-token' || account?.type === 'apikey')"
        class="space-y-4 border-t border-stone-200/80 pt-4 dark:border-white/10"
      >
        <div class="flex items-center justify-between">
          <div>
        ...
        <div class="rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-600 dark:bg-white/[0.06] dark:text-gray-300">
        ...
        <div v-if="account?.platform === 'antigravity'" class="flex items-center gap-2">
          <label class="flex cursor-not-allowed items-center gap-2 opacity-60">
            <input
              ...
              disabled
              class="h-4 w-4 cursor-not-allowed rounded border-stone-300 text-emerald-500 focus:ring-emerald-500 dark:border-white/15"
            />
...
const loadModelRestrictionFromMapping = (
  ...
  const entries = rawMapping && typeof rawMapping === 'object'
  ...
  if (credentials.model_restriction_mode === 'mapping') {
  ...
  if (credentials.model_restriction_mode !== 'whitelist' && parsed.modelMappings.length > 0 && parsed.allowedModels.length === 0) {
    modelRestrictionMode.value = 'mapping'
  ...
  if (shouldApplyModelMapping) {
    ...
    if (modelMapping) {
      ...
      if (!isSparkShadow.value) {
        ...
        delete credentials.model_restriction_mode
      }
    } else {
    ...
    // Passthrough mode preserves any existing mapping and restriction mode.
  } else {
    delete credentials.model_mapping
    delete credentials.model_restriction_mode
  }
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    loadModelRestrictionFromMapping(credentials.model_mapping as Record<string, unknown> | undefined, credentials)
    ...
    loadModelRestrictionFromMapping(bedrockCreds.model_mapping as Record<string, unknown> | undefined, bedrockCreds)
    ...
    if ((newAccount.platform === 'openai' || newAccount.platform === 'grok') && newAccount.credentials) {
      ...
      loadModelRestrictionFromMapping(oauthCredentials.model_mapping as Record<string, unknown> | undefined, oauthCredentials)
    } else {
      modelRestrictionMode.value = 'whitelist'
      modelMappings.value = []
      allowedModels.value = []
