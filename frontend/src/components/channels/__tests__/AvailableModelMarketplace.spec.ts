...
describe('AvailableModelMarketplace', () => {
  ...
  it('separates model cards by group and keeps each group route inside its own section', () => {
    ...
    expect(wrapper.text()).not.toContain('availableChannels.modelMarketplace.channelCount')
    expect(wrapper.text()).not.toContain('availableChannels.modelMarketplace.availableChannels')
    expect(publicSection.text()).not.toContain('ikuncode-cx')
    expect(publicSection.text()).not.toContain('智链')
    ...
    expect(exclusiveSection.text()).not.toContain('专属线路')
    expect(exclusiveSection.text()).not.toContain('ikuncode-cx')
    expect(exclusiveSection.text()).toContain('/v1/messages')
    expect(exclusiveSection.text()).not.toContain('/v1/chat/completions')
    expect(exclusiveSection.text()).not.toContain('/v1/responses')
  })
