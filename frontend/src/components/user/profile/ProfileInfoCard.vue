    ...
    >
      <div class="px-6 py-6 md:px-8">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-start">
          <div
            class="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-[1.75rem] bg-gradient-to-br from-emerald-400 to-teal-500 text-2xl font-bold text-black shadow-lg shadow-emerald-500/15"
          >
            <img
              v-if="avatarUrl"
              :src="avatarUrl"
          ...
          <div class="min-w-0 flex-1 space-y-5">
            <div class="space-y-3">
              <div class="flex flex-wrap items-center gap-2">
                <h2 class="truncate text-2xl font-semibold text-stone-950 dark:text-white">
                  {{ displayName }}
                </h2>
              ...
              <div class="space-y-1">
                <p class="truncate text-sm text-stone-600 dark:text-stone-300">
                  {{ primaryEmailDisplay }}
                </p>
                <div
                  v-if="sourceHints.length"
                  class="flex flex-wrap gap-2 text-xs text-stone-500 dark:text-stone-500"
                >
                  <span
                    v-for="hint in sourceHints"
                    :key="hint.key"
                    class="inline-flex items-center gap-1 rounded-full bg-white/80 px-3 py-1 ring-1 ring-stone-200/80 dark:bg-black/30 dark:ring-white/10"
                  >
            ...
            <div class="grid gap-3 sm:grid-cols-3">
              <div
                ...
                class="rounded-xl border border-stone-200/80 bg-white/70 px-4 py-3 dark:border-white/10 dark:bg-black/25"
              >
                <p class="text-xs font-medium uppercase tracking-[0.16em] text-stone-400 dark:text-stone-500">
      ...
      <div data-testid="profile-main-column" class="space-y-6">
        ...
        >
          <div class="mb-5 flex items-start justify-between gap-4">
            <div>
              <h3 class="text-lg font-semibold text-stone-950 dark:text-white">
          ...
          <div class="grid gap-6 sm:grid-cols-1 md:grid-cols-2">
            <div class="rounded-xl border border-stone-200/80 bg-white/65 p-5 dark:border-white/10 dark:bg-black/25">
              <ProfileAvatarCard
                :user="user"
                embedded
              />
        ...
        >
          ...
          <div class="mt-5 grid gap-3">
            <div
              v-for="hint in sourceHints"
              :key="hint.key"
              class="flex items-start gap-3 rounded-xl border border-stone-200/80 bg-white/65 px-4 py-3 text-sm text-stone-600 dark:border-white/10 dark:bg-black/25 dark:text-stone-300"
            >
              <Icon name="link" size="sm" class="mt-0.5 text-stone-400 dark:text-stone-500" />
              <span>{{ hint.text }}</span>
            </div>
          </div>
        </section>
