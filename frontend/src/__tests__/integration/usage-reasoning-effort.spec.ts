...
describe('usage reasoning effort page display', () => {
  ...
  it('admin usage page shows requested and mapped effort after the column is enabled', async () => {
    ...
    const reasoningToggle = document.body.querySelector('[data-testid="usage-column-toggle-reasoning_effort"]')
    expect(reasoningToggle).not.toBeNull()
    await new DOMWrapper(reasoningToggle as Element).trigger('click')
    await flushPromises()
