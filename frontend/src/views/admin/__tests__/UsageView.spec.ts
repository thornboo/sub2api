...
const { list, exportList, getStats, searchApiKeys, getSnapshotV2, getById, getUserApiKeys, getModelStats, listErrorLogs, routerReplace, routeQuery, aoaToSheet, sheetAddAoa, saveAs, xlsxWrite } = vi.hoisted(() => {
...
describe('admin UsageView model audit export', () => {
	...
	it('exports requested, sent, response, and mismatch as separate admin columns', async () => {
		...
		expect(headers.slice(7, 11)).toEqual([
			'Requested model',
			'Sent upstream model',
		...
		expect(row.slice(7, 11)).toEqual(['gpt-5.6-sol', 'gpt-5.5', 'gpt-5.4', 'Yes'])
