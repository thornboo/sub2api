...
<template>
  <section class="card p-4">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-bold text-stone-950 dark:text-white">系统日志</h3>
        <p class="mt-1 text-xs text-stone-500 dark:text-stone-400">默认按最新时间倒序，支持筛选搜索与按条件清理。</p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-stone-100 px-2 py-1 text-stone-700 dark:bg-white/10 dark:text-stone-200">队列 {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-stone-100 px-2 py-1 text-stone-700 dark:bg-white/10 dark:text-stone-200">写入 {{ health.written_count }}</span>
    ...
    <div class="mb-4 rounded-xl border border-stone-200/80 bg-white/65 p-3 dark:border-white/10 dark:bg-black/25">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-stone-700 dark:text-neutral-200">运行时日志配置（实时生效）</div>
        <span v-if="runtimeLoading" class="text-xs text-stone-500">加载中...</span>
      </div>
    ...
    <div class="overflow-hidden rounded-xl border border-stone-200/80 dark:border-white/10">
      <div v-if="loading" class="px-4 py-8 text-center text-sm text-stone-500">加载中...</div>
      <div v-else-if="!hasData" class="px-4 py-8 text-center text-sm text-stone-500">暂无系统日志</div>
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-stone-200/80 dark:divide-white/10">
          <thead class="bg-stone-50/90 dark:bg-neutral-950">
            <tr>
              <th class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-stone-500">时间</th>
              <th class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-stone-500">级别</th>
              <th class="px-3 py-2 text-left text-[11px] font-semibold text-stone-500">日志详细信息</th>
            </tr>
          </thead>
