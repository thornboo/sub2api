...
function severityBadgeClass(severity: string | undefined): string {
  ...
  if (s === 'p2' || s === 'info') return 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300'
  if (s === 'p3') return 'bg-stone-100/80 text-stone-700 dark:bg-white/[0.08] dark:text-neutral-300'
  return 'bg-stone-100/80 text-stone-700 dark:bg-white/[0.08] dark:text-neutral-300'
}
...
function statusBadgeClass(status: string | undefined): string {
  ...
  if (s === 'resolved') return 'bg-emerald-50 text-emerald-700 ring-emerald-600/20 dark:bg-emerald-950/40 dark:text-emerald-300 dark:ring-emerald-500/30'
  if (s === 'manual_resolved') return 'bg-stone-50 text-stone-700 ring-stone-600/20 dark:bg-white/[0.08] dark:text-neutral-300 dark:ring-white/15'
  return 'bg-stone-50/80 text-stone-700 ring-stone-600/20 dark:bg-white/[0.08] dark:text-neutral-300 dark:ring-white/15'
}
...
<template>
  <div class="card p-6">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
      <div>
        <h3 class="text-sm font-bold text-stone-950 dark:text-white">{{ t('admin.ops.alertEvents.title') }}</h3>
        <p class="mt-1 text-xs text-stone-500 dark:text-stone-400">{{ t('admin.ops.alertEvents.description') }}</p>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <button
          class="btn btn-secondary btn-sm gap-1.5 px-3 py-1.5 text-xs font-bold"
          :disabled="loading"
          @click="loadFirstPage"
        >
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-stone-200 p-8 text-center text-sm text-stone-500 dark:border-white/10 dark:text-stone-400">
      ...
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <div v-if="!isDesktopViewport" class="divide-y divide-stone-200/70 dark:divide-white/10">
          <div
            v-for="row in events"
            :key="row.id"
            class="cursor-pointer space-y-2 p-4 hover:bg-stone-50/80 dark:hover:bg-white/[0.04]"
        ...
        <table v-else class="min-w-full divide-y divide-stone-200/80 dark:divide-white/10">
          <thead class="sticky top-0 z-10 bg-stone-50/90 dark:bg-neutral-950">
            <tr>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-stone-500 dark:text-stone-400">
              ...
              <th class="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-stone-500 dark:text-stone-400">
                {{ t('admin.ops.alertEvents.table.email') }}
              </th>
            </tr>
          </thead>
          <tbody class="divide-y divide-stone-200/80 bg-white dark:divide-white/10 dark:bg-neutral-950">
            <tr
              v-for="row in events"
              :key="row.id"
              class="cursor-pointer hover:bg-stone-50/80 dark:hover:bg-white/[0.06]"
              @click="openDetail(row)"
              :title="row.title || ''"
            >
              <td class="whitespace-nowrap px-4 py-3 text-xs text-stone-600 dark:text-neutral-300">
                {{ formatDateTime(row.fired_at || row.created_at) }}
              </td>
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-stone-700 dark:text-neutral-200">
                <div class="font-semibold truncate max-w-[360px]">{{ row.title || '-' }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-stone-500 dark:text-stone-400">
                  {{ row.description }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-stone-600 dark:text-neutral-300">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-[11px] text-stone-500 dark:text-stone-400">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-right text-xs">
                ...
                >
                  ...
                  />
                  <span class="text-[11px] font-bold text-stone-600 dark:text-neutral-300">
                    {{ row.email_sent ? t('admin.ops.alertEvents.table.emailSent') : t('admin.ops.alertEvents.table.emailIgnored') }}
                  </span>
                </span>
              </td>
        ...
        <div v-if="loadingMore" class="flex items-center justify-center gap-2 py-3 text-xs text-stone-500 dark:text-stone-400">
          <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        ...
        <div v-else-if="!hasMore && events.length > 0" class="py-3 text-center text-xs text-stone-400">
          -
        </div>
      </div>
    </div>
    ...
    <BaseDialog
      ...
      @close="closeDetail"
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-stone-500 dark:text-stone-400">
        {{ t('admin.ops.alertEvents.detail.loading') }}
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-stone-500 dark:text-stone-400">
        {{ t('admin.ops.alertEvents.detail.empty') }}
      </div>

      <div v-else class="space-y-5">
        <div class="rounded-xl bg-stone-50/80 p-4 dark:bg-black/30">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div class="mt-2 text-sm font-semibold text-stone-950 dark:text-white">
                {{ selected.title || '-' }}
              </div>
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap text-xs text-stone-600 dark:text-neutral-300">
                {{ selected.description }}
              </div>
            </div>

            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-white px-2 py-1 ring-1 ring-stone-200/80 dark:bg-neutral-950 dark:ring-white/10">
                <span class="text-[11px] font-bold text-stone-600 dark:text-neutral-300">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-stone-50/80 p-4 dark:bg-black/30">
              <div class="text-xs font-bold uppercase tracking-wider text-stone-400">{{ t('admin.ops.alertEvents.detail.firedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-stone-950 dark:text-white">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-xl bg-stone-50/80 p-4 dark:bg-black/30">
              <div class="text-xs font-bold uppercase tracking-wider text-stone-400">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-stone-950 dark:text-white">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-xl bg-stone-50/80 p-4 dark:bg-black/30">
              <div class="text-xs font-bold uppercase tracking-wider text-stone-400">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-bold text-stone-950 dark:text-white">#{{ selected.rule_id }}</div>
                <a
                  class="inline-flex items-center gap-1 rounded-md bg-white px-2 py-1 text-[11px] font-bold text-stone-700 ring-1 ring-stone-200/80 hover:bg-stone-50/80 dark:bg-neutral-950 dark:text-neutral-200 dark:ring-white/10 dark:hover:bg-white/[0.08]"
            ...
            <div class="rounded-xl bg-stone-50/80 p-4 dark:bg-black/30">
              <div class="text-xs font-bold uppercase tracking-wider text-stone-400">{{ t('admin.ops.alertEvents.detail.dimensions') }}</div>
              <div class="mt-1 text-sm text-stone-950 dark:text-white">
        ...
        <div class="rounded-xl border border-stone-200/80 bg-white p-4 dark:border-white/10 dark:bg-neutral-950">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-sm font-bold text-stone-950 dark:text-white">{{ t('admin.ops.alertEvents.detail.historyTitle') }}</div>
              <div class="mt-0.5 text-xs text-stone-500 dark:text-stone-400">{{ t('admin.ops.alertEvents.detail.historyHint') }}</div>
            </div>
            <Select :model-value="historyRange" :options="historyRangeOptions" class="w-[140px]" @change="historyRange = String($event || '7d')" />
          </div>

          <div v-if="historyLoading" class="py-6 text-center text-xs text-stone-500 dark:text-stone-400">
            {{ t('admin.ops.alertEvents.detail.historyLoading') }}
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-stone-500 dark:text-stone-400">
            {{ t('admin.ops.alertEvents.detail.historyEmpty') }}
          </div>
          <div v-else class="overflow-hidden rounded-lg border border-stone-200/70 dark:border-white/10">
            <table class="min-w-full divide-y divide-stone-200/70 dark:divide-white/10">
              <thead class="bg-stone-50/80 dark:bg-black/30">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-stone-500 dark:text-stone-400">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-stone-500 dark:text-stone-400">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-stone-500 dark:text-stone-400">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-stone-200/70 dark:divide-white/10">
                <tr v-for="it in history" :key="it.id" class="hover:bg-stone-50/80 dark:hover:bg-white/[0.06]">
                  <td class="px-3 py-2 text-xs text-stone-600 dark:text-neutral-300">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
