...
Sub2API 已内置支付系统，用户可以自助充值，不需要再单独部署支付服务。
...
当前版本面向用户只展示支付方式，不展示背后的服务商来源：

- **支付宝**：后台启用后，还需要指定这个按钮走 `支付宝官方` 还是 `易支付支付宝`
- **微信支付**：后台启用后，还需要指定这个按钮走 `微信官方` 还是 `易支付微信`
...
不同服务商需要填写不同凭证和参数。在 **服务商管理 → 添加服务商** 中选择类型后填写。
...
> **注意**：Sub2ApiPay 里的历史订单数据不会自动迁移。建议保留 Sub2ApiPay 一段时间，方便查询历史记录。
