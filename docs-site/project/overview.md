...
Sub2API 是一个面向 AI API 转发、订阅额度分发、多账号管理和用量计费的网关平台。后端负责认证、账号调度、额度、用量记录和支付回调；前端提供用户控制台和管理控制台。
...
当前 checkout 里的 `dev-zz` 是二次开发分支。它会持续吸收上游 `main` 的修复，同时保留本分支已经形成的企业 Key 管理、owner 用量分析、模型/渠道展示、部署镜像、运维体验和控制台视觉策略。

二开内容从 [dev-zz 文档](../dev-zz/index.md) 开始看即可。想快速了解 dev-zz 相对上游改了哪些地方，优先看 [变更地图](../dev-zz/reference/change-map.md)。
