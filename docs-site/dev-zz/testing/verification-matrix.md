...
| 永久删除成员退出 owner 成员统计、趋势、请求与错误记录（含空范围、全部、指定成员与按 ID 详情），归档成员继续保留，admin/audit 仍可查历史 | `cd backend && go test ./internal/repository ./internal/service ./internal/handler -run 'Test(AppendUsageLogMember(WhereCondition\|QueryFilterPreservesAllOwnerConditions)\|OwnerAnalytics(AssignedMembersExcludeRemovedTombstones\|AllMembersExcludeRemovedTombstonesWithoutForcingAssignment\|EmptyMemberScopeStillExcludesRemovedTombstones)\|OwnerMember(AssignedUsageConditionsExcludeRemovedTombstones\|AllUsageConditionsExcludeRemovedTombstones)\|BuildOpsErrorLogsWhere_(AssignedMembersExcludeRemovedTombstones\|OwnerAllAndEmptyScopesExcludeRemovedTombstones\|OwnerSpecificMemberRequiresCurrentVisibility\|Admin(AllScopeRetainsAuditHistory\|AssignedScopeRetainsRemovedHistory))\|EnterpriseMemberOwnerUsageTrendExcludesRemovedMembers\|OwnerVisibleEnterpriseMemberFactOrUnassignedCondition\|UsageLogRepositoryGetByIDForOwnerRequiresVisibleOrUnassignedMember\|UsageLogRepositoryGetByIDRetainsUnfilteredAuditLookup\|OpsRepositoryGetErrorLogByIDForOwnerRequiresVisibleOrUnassignedMember\|OpsRepositoryGetErrorLogByIDRetainsUnfilteredAuditLookup\|UsageServiceGetByIDForOwnerUsesOwnerVisibleRepositoryQuery\|GetUserErrorRequestDetail_OwnershipEnforced\|ListUserErrorRequests_ForcesScopeAndRedacts\|UserUsageDashboardModelsAlwaysUsesCompleteFilters)$' -count=1` |
...
| 企业成员模型感知资格规划、Composite 文本协议预览、非文本 evaluator coverage、enforce readiness、LKG/revision、typed attempt、Ops routing attempts、alias review 与 phase5 退役准备 | 最终本地验证已 PASS：后端 `go generate`、`make test-unit`、`go test ./... -count=1`、`go vet ./...`、`golangci-lint run ./...`（0 issues），18 个 Colima/Testcontainers PostgreSQL 16/Redis 顶层测试（14 个 routing eligibility + 4 个 migration runner），focused race，前端 typecheck/lint/完整测试（259 个测试文件、1758 条测试）/build，docs build，`git diff --check`，隐私/脱敏专项测试及新增差异的高置信度密钥模式扫描；未把环境中未安装的 gitleaks 等专用扫描器计作已执行证据。分阶段命令和故障注入门见 [企业成员模型感知路由实施测试规格](./enterprise-member-model-aware-routing-test-spec.md)；Composite 定向验证使用 `cd backend && go test ./internal/service -run 'Test(CompositeRouteResolverPreview\|EnterpriseMemberModelDeliveryComposite\|EnterpriseMemberRoutePlannerUsesSideEffectFreeCompositePreview)' -count=1`；revision/LKG 单元门使用 `cd backend && go test ./internal/service ./migrations -run 'Test(RoutingEligibilityRuntime\|RoutingEligibilityRevision\|RouteSnapshot\|EnterpriseMemberRoutePlannerRestores\|EnterpriseMemberRoutePlannerRejectsLKG\|EnterpriseMemberModelDeliveryAppliesStableGroupAccountPolicies\|RoutingEligibilityRevisionMigration)' -count=1`；迁移 199-202（含 `201a` 非事务索引合同）使用 `cd backend && go test ./migrations -run 'Test(RoutingEligibilityRevisionMigration\|EnterpriseMemberAliasReviewLedgerMigration\|OpsRoutingAttemptsMigration\|ModelProtocolCapabilitiesNonTextMigrationExtendsProtocolCheck)' -count=1`；alias/Ops/admission 管理能力使用测试规格第 8-10 节命令。`201` 历史 CHECK 验证和 `201a` 生产同量级演练仍是发布门。commit、release、deploy、生产 7d/30d/canary release-window 证据、默认 enforce、shadow 删除和 legacy 退役仍必须等真实生产发布窗口证据，不能由本地测试单独关闭。 |
