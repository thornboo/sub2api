...
> 状态：部分落地。阶段 1 后端兼容层已实现：`166_upstream_cost_pools.sql` 新增供应商、资金池、账号成本绑定和成本快照表；旧账号级充值接口保留，但会写入账号默认资金池。本期账本只支持 `recharge` / `bonus` / `adjustment` 三类非负金额记录，且只有 `recharge` / `bonus` 会生成成本快照。资金池管理页、显式合池、退款 / 冲正 / 作废账本、余额查询迁移、usage 成本证据和调度联动仍是后续阶段。
...
- 充值账本属于资金池，不属于账号。
- 余额查询属于资金池，不属于账号。
- 账号只表达它如何消费某个资金池，例如平台、倍率、模型族覆盖和调度参数。
...
阶段 1 已落地的兼容接口只接受 `recharge` / `bonus` / `adjustment`，金额仍要求非负；`refund` / `correction` / `void` 和负向调整属于后续账本语义扩展。成本快照只从 `recharge` / `bonus` 计算，避免运营调整记录污染资金池当前成本。
...
| `current_effective_cny_per_usd` | 当前展示和调度建议使用的基础成本 |
...
| `account_id` | 绑定的上游账号 |
| `cost_pool_id` | 绑定的资金池 |
| `default_multiplier` | 该账号默认消费倍率，吸收原「上游分组倍率」语义，不再单列分组倍率字段 |
...
```text
编辑账号 -> 选择上游供应商 -> 必要时选择该供应商下的钱包 / 资金池
```

产品契约：

- 管理员第一次配置账号时，可以在账号编辑弹窗内点击“新增”，先把“供应商 A”新增到供应商列表。
- 新增供应商时，系统默认创建一个“主余额池”；账号是否绑定到该供应商，仍以表单里最终选中的下拉值和账号保存动作为准。
...
- 阶段 1 账号编辑页的主字段只应该是“供应商”。真实充值成本来自供应商 / 资金池账本，不再让管理员在每个账号上重复录入真实充值、分组倍率或模型系列倍率。
...
```text
编辑账号 -> 新增供应商 A 到列表 -> 下拉框选择供应商 A -> 保存账号 -> 账号绑定到供应商 A / 主余额池
```

再次使用流程：

```text
编辑另一个账号 -> 选择供应商 A -> 默认绑定供应商 A / 主余额池 -> 保存
```
...
旧版“充值人民币/美元额度、参考汇率、默认分组倍率、模型系列倍率”字段可以短期保留在兼容代码和历史数据里，但不应展示在新版账号编辑主流程里。新版主交互应让管理员先选供应商，再由系统处理默认资金池、绑定、账本和快照。

## API 设计

本节分为当前已落地接口和长期目标接口。未列入“当前已落地”的创建供应商、编辑资金池、作废账本、余额刷新和快照重算接口仍是后续工作。
...
- 为已有账号创建“未归类供应商”下的账号默认资金池和 active 绑定。
...
2. 为每个已有账号自动创建一个默认供应商和默认资金池，或创建一个“未归类供应商”下的账号默认资金池。
