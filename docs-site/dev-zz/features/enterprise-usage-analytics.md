...
> 状态：已按增量兼容方案落地。现有 owner API Key 用量分析保持兼容；企业账号使用独立的“成员使用记录”入口查看成员排行、请求与错误归因，原“使用记录”只保留企业账号普通 Key 的事实。
...
- 企业账号的“成员使用记录”以成员作为唯一一级对象：成员筛选统一驱动统计、图表、请求明细和错误记录，成员排行负责成员下钻；页面不提供 Key 维度切换或 Key 主筛选，Key 仅作为请求来源保留在明细和导出证据中。
- 用户认证域已经提供：
...
- 用户侧 DTO 不返回 `account_id`、`account_cost`、上游账号、渠道或真实上游路由。
...
- 成员是“成员使用记录”的一级筛选与聚合对象，解决一名成员多 Key 的总量归集。
- Key 是成员请求明细中的次级来源信息，不在成员模块中提供独立排行或主筛选；需要调查单把成员 Key 时，从成员管理的 Key 详情进入。
- 实际分组是成员请求的分析维度，记录请求级 `ActiveGroup`，支持多分组成员的用量拆分。
...
成员排行只包含真实 `enterprise_members` 实体，并按 `usage_logs.member_id` 聚合请求事实；成员当前名称用于列表，历史名称快照用于审计。`member_id IS NULL` 的普通 Key 请求继续保留在总用量、Key 分析、请求记录和错误记录中，但不得伪造成成员排行项；因此 `/usage/analytics/members?member_scope=unassigned` 返回空成员集合。

`GET /api/v1/usage/analytics/members` 的汇总字段基于完整筛选结果计算，不受前端 `limit` 截断：

- `total`：符合筛选条件的真实成员排行对象数。
- `member_count`：真实成员数；与 `total` 使用同一成员实体口径。
...
1. “使用记录”承载企业 owner 自己直接使用普通 Key 产生的请求。
2. “成员使用记录”承载成员排行、预算风险、趋势、模型、分组和请求明细。
3. 成员请求明细保留 Key 名称与 ID，作为请求来源和审计证据。

成员统计始终先聚合成员名下所有 Key，不把一名成员多 Key 拆成多名员工。

成员使用记录页的交互约束：

- 成员是唯一一级分析维度，不提供“按 Key”切换、Key 排行或 Key 主筛选。
- 成员筛选位于页面统计与图表之前，并统一驱动统计、分析、请求明细和错误记录。
...
- 历史成功记录只有在不可变成员预算账本能同时证明 member、Key 与 request identity 时才回填 `member_id`；无法证明请求时成员名称或编号的旧记录保持空快照，禁止从当前成员行反向伪造历史。
...
- 跨企业成员 ID 被拒绝。
- 一成员多 Key 正确合并。
- 普通 Key 请求只进入 `unassigned` 请求事实范围，不污染任意成员，也不生成虚拟成员排行项。
- 一 Key 多实际分组正确拆分。
...
- owner/member DTO 不出现 `account_id`、`account_cost`、上游账号、渠道、上游 endpoint 或利润。

### 9.3 前端

- individual/enterprise/admin 三视角正确。
- 成员 → request 下钻保持筛选和日期，Key 作为请求来源证据随明细展示。
