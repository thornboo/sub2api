...
- 状态：阶段 1 已落地（对应提交 `7bd82caf`）；阶段 2、阶段 3 仍为设计草案，待审查。
- 范围：API Key 删除后，管理员和用户侧用量明细、统计分析、导出和对账证据的完整性。
- 背景页面：
  - 管理员使用记录：`/admin/usage`
  - 用户侧 API Key 管理：`/keys`
  - 用户侧用量分析：`/usage`
- 非范围：本文不调整扣费算法、网关转发、限流、余额变动、订阅用量更新或历史清理策略。

本文的核心判断是：`usage_logs` 应被视为不可变消费账本，`api_keys`、`users`、`groups`、`accounts` 等表只是可变维度。消费证据不能因为维度对象后续改名、禁用、删除而失去可解释性。

阶段 1 已按本文设计落地：`/admin/usage` 管理员证据视图穿透软删除解析已删除 Key 的名称和删除状态，DTO 隐藏明文 key 并向管理员证据上下文暴露删除元数据，导出补充 Key ID、名称和删除时间；用户侧 `/usage` 和普通 `/keys` 列表仍只解析活跃 Key。阶段 2（快照字段）和阶段 3（外键约束）保持设计草案，实施前需按各阶段前置核验执行。
