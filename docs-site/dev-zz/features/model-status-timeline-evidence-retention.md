...
> 状态：分阶段落地中。设计 2026-07-02，阶段 1 已落地。2026-09-07 已补齐列表与详情历史指标的批量快照聚合；当前状态仍按合格账号即时聚合，时间桶化展示仍为后续阶段。
>
> 下文保留原始设计背景和阶段划分；最新资格判断、历史指标回退和当前探测元数据口径见[模型状态 V1 问题 2 实施记录](./model-status-v1-bug-analysis-20260907.md)。原设计中的账号级时间线问题已通过阶段 1 快照解决。
