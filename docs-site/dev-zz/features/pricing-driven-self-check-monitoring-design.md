...
- 用户侧只在**当前用户可用分组**内，以 **`分组/模型`** 维度展示健康状态；公开标准分组、已授权专属分组和有效订阅分组复用 API Key 分组绑定口径，但**不暴露** provider、endpoint、账号、上游模型、原始错误、成本。
...
- **展示层（给用户）**：以当前用户可用的 `分组/模型` 为单位。公开标准分组对所有用户可见；专属分组需要明确授权；订阅分组需要有效订阅。布局按可用分组分区：
...
- 用户接口需登录，并复用 `APIKeyService.GetAvailableGroups` 将列表与详情限制在当前用户的公开标准分组、已授权专属分组和有效订阅分组内；未授权详情按不存在处理。返回 `group_name`/`model`/`status`/可用率/延迟/时间线/降级比例；**绝不返回** `account_id`/`platform`/`provider`/`endpoint`/`upstream_model`/原始错误/`cost`/`channel_id`。
...
- 用户分组隔离：公开标准分组可见；未授权专属分组和无有效订阅的订阅分组不进入列表，直接请求其详情返回不存在。
