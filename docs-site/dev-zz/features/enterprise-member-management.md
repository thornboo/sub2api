...
导入结果返回本次创建的成员 ID，以及导入时冻结的 `period_start` 与 `timezone`；完成页必须继续显示该归属期，不能用浏览器当前月份重新解释迁移金额。owner 可以直接对本批成员执行“批量设置可访问分组”；批量操作支持按明确模式覆盖或追加，默认使用覆盖且必须展示影响范围。批量设置分组不会静默启用成员，启用必须是独立、明确的 owner 操作；覆盖为空会清空路由权限并同步停用成员，提交前必须使用危险操作确认框回显受影响成员数量。
...
已完成 import job 与迁移用量基线属于长期可审计证据。正常 owner 操作不提供物理删除入口；如未来引入独立的数据保留或合规清理流程，必须显式定义 job、成员、Key、预算账本和基线之间的整组证据处理语义，不能通过放宽外键或级联删除隐式实现。
