...
2026-08-05 的生产样本确认发布版实现尚未完全满足本节合同：请求模型没有参与初始候选资格，导致无关授权分组被顺序激活，终态错误被最后一个候选覆盖。根因、模型准入策略、共享稳定资格评估、typed attempt、Ops 路由链、alias 审核和灰度方案见 [企业成员模型感知路由与跨分组故障转移治理](./enterprise-member-model-aware-routing-governance.md)。2026-08-13 的后续修复在不启用账号/协议完整规划的前提下增加了正向发布裁剪：只从 ChannelService 的十分钟缓存和 singleflight 快照中选择明确发布该模型的当前授权分组，至少命中一个才缩小候选，依赖失败、零命中或越权结果均 fail-open；同优先级终态失败改为保留成员顺序中最早的失败，避免后续空池分组覆盖真实归因。该修复仍需 commit、CI、release 和生产窗口验证，且不代表 `enforce_published` readiness 已满足；它不改变“成员以分组为授权单位”的领域边界。
