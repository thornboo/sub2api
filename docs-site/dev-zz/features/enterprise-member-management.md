...
| 列 | 必填 | 说明 |
| --- | --- | --- |
| `member_code` | 是 | 企业内稳定 ID |
| `member_name` | 是 | 展示名 |
| `rate_limit_5h` | 否 | 成员全部 Key 共享的 5 小时限额，0/空表示不限 |
| `rate_limit_1d` | 否 | 成员全部 Key 共享的 24 小时限额，0/空表示不限 |
| `rate_limit_7d` | 否 | 成员全部 Key 共享的 7 天限额，0/空表示不限 |
| `monthly_limit_usd` | 否 | 0/空表示不限 |
| `opening_used_usd` | 否 | 迁移当月开账，只能在同一成员首行填写 |
| `key_name` | 否 | Key 名称 |
| `api_key` | 否 | 外部明文 Key；空则生成 |
| `key_quota_usd` | 否 | 单 Key quota |
| `groups` | 是 | 有序 group ID/code 列表；不以易变展示名作为唯一解析依据 |

XLSX 模板使用三个 sheet：

- `Members`：member code、名称、预算、开账。
- `Keys`：member code、key name、明文 Key、quota。
- `MemberGroups`：member code、group ID、sort order。
