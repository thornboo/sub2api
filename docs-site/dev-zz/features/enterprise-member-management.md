...
任一限额为 0 只表示该窗口不限；只要四个成员限额中有一个启用，请求就必须预留保守上界。四个限额全部为 0 时仍创建金额为 0 的 durable receipt，使崩溃恢复、请求幂等、账本月份和 usage 归因保持同一条证据链。

模型查询、用量查询、`count_tokens` 与异步任务状态查询不产生对客费用，不创建成员预算 receipt；视频生成、编辑和扩展等实际媒体任务必须创建 receipt。
...
- Responses WebSocket 不为整条长连接创建一个不可解释的预留；每个 `response.create` 使用独立请求 ID、请求体和模型创建 reservation。成功 turn 按真实用量结算；明确未到达上游或上游返回结构化拒绝的失败才能释放。`response.create` 一旦进入上游写入边界，写入超时/断连、读取中断或缺失 terminal 事件都必须转为 `ambiguous` 待对账，不得换连或回退 HTTP 重放。
...
若最终实际费用意外超过保守预留值，必须记录告警并按真实费用原子写入 usage、账本和投影；不能回滚已经发生的请求事实。后续请求由已经超限的投影正常阻断。

### 8.6 失败与取消

- 所有分组均失败且不计费：释放 reservation，状态为 `released`。
- 客户端断开但上游已产生可计费用量：按现有计费事实结算，不因客户端断开丢账。
- worker 崩溃留下的 `reserved` 由恢复任务按 `expires_at` 检查实际 usage/billing 事实：证据完整则结算；能够证明请求未执行才释放；结果无法证明则转为 `ambiguous` 并持续保留预算。
...
成员 Key 列表只返回脱敏值，避免打开管理弹窗时批量暴露全部凭据；用户主动点击复制时，前端通过 owner 已有的普通 Key 详情接口按 ID 读取单把 Key，并复用统一剪贴板降级与反馈逻辑。创建成功响应仍会立即展示一次明文，审计与用量日志永不返回完整 Key。
...
| `总消耗Token数` | `total_tokens` | 否 | 外部系统给出的汇总值；与分项不一致时预览警告但保留原值；未提供时统一以输入 + 输出作为有效总量 |
| `总输入Token数` | `input_tokens` | 否 | 当前统计周期的迁移期初输入 Token |
| `总输出Token数` | `output_tokens` | 否 | 当前统计周期的迁移期初输出 Token |
| `总缓存Token数` | `cache_tokens` | 否 | 外部系统给出的缓存汇总 |
| `总缓存Token写入数` | `cache_creation_tokens` | 否 | 迁移期初缓存写入 Token |
| `总缓存Token读取数` | `cache_read_tokens` | 否 | 迁移期初缓存读取 Token |
...
- Token 汇总必须为非负整数；来源提供总量时保留该值并在与已知分项不一致时给出稳定警告；来源未提供总量时，导入结果、不可变基线和后续预算摘要统一以输入 + 输出作为有效总量，缓存分项独立展示且不重复加入总量。
- 当前周期和站点计费时区。
