...
- 当前成员的名称、成员编号、月预算、实际已用、剩余、有限超额状态和共享窗口限制；不展示内部 receipt、hold 或历史 `reserved_usd`。
...
返回会话是否有效、过期时间，以及有效会话的非凭据 `session_id`（用于前端公告展示去重），不返回 Key、Cookie 令牌或用户／Key 内部 ID。
...
Key 验证成功或恢复有效查询会话后，页面提供公告入口。未建立查询会话时不加载公告。

```text
GET  /api/v1/key/announcements
POST /api/v1/key/announcements/:id/read
```

- 两个接口均复用查询 Cookie 会话，服务端解析当前 Key 和所属用户；不接受客户端指定其他 Key 或用户。
- 公告沿用发布状态、生效／结束时间，以及按所属用户余额和有效订阅判断的定向规则。订阅条件不改成 Key 可用分组条件；接口不返回用户余额、订阅或管理员定向配置。
- `popup` 自动展示未读公告，逐条展示且同一查询会话内去重；关闭而未标记已读的公告，在同一标签页离开后返回也不会重复自动弹出，仍可从列表查看。`silent` 仅进入公告列表和未读提示。
- 有效查询会话响应提供非凭据 `session_id`，仅用于隔离前端展示记录，不能代替 HttpOnly Cookie 鉴权。标签页会话存储只保存该标识和已展示公告 ID；存储不可用时使用内存回退，跨路由导航仍可去重。
- 已读记录按 Key 单独持久化。同一 Key 的查询会话共享已读；同一用户的不同 Key 互不影响，也不更新登录用户的已读记录或管理员的用户阅读统计。
- 标记已读前重新检查公告可见性，过期、下线或不可见公告不能通过猜测 ID 标记；重复标记保持幂等。
- Key 页面与登录控制台的公告队列隔离。卸载清理视图、队列与在途请求，但保留会话展示记录；切换 Key、退出查询或会话失效时清除对应记录。公告加载失败不阻断用量查询。
- 内容复用 Markdown 安全清洗。公告列表和弹窗遵循简洁中性样式，支持窄屏换行和纵向滚动。

本次不增加匿名公告范围，不新增公告配置开关，不改变现有余额／订阅定向的含义。
