...
- 完整配置使用独立弹窗，便于同步和批量查看当前账号实际路由的模型。
...
- 最近观察时间。
- 影响的渠道公开模型和分组。
- 映射目标暂未被任何渠道模型使用时的路由警告。

只允许管理员编辑覆盖状态；观察结果只读。账号存在显式 `model_mapping` 时，模型能力列表只包含映射右侧的最终上游目标以及账号默认能力 `*`，不会展示同一上游目录中未参与该账号路由的其他模型。空映射仍表示允许全部模型，此时保留完整上游同步结果和手工添加精确模型的能力。
...
- “同步”只更新观察结果。
- 不把同步结果隐式变成手动配置。
- 显式模型映射账号只同步映射右侧的最终上游目标；目录中的其他模型不写入本次观察。
- 未知枚举显示警告，但不阻止模型 ID 同步。
- 字段缺失时明确显示“上游未声明协议能力”，而不是“不支持”。
- 保存覆盖成功后关闭弹窗；保存失败时保留当前编辑状态并显示错误。
