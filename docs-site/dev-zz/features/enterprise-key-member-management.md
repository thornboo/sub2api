...
- `POST /api/v1/keys/batch-update`：当前已支持按 `ids` 批量改 `status` / `quota` / `rate_limit_*` / `expires_at` / `group_id` / IP ACL / `tags`，并支持重置限流窗口用量。标签更新支持 `set` / `add` / `remove` / `clear`。
- `POST /api/v1/keys/batch-delete`：当前已支持按 `ids` 批量软删除。
- 用户侧 Key 列表当前支持按 `tags` 查询参数筛选，多个标签语义为同时包含。
- 按 `filters`（分组/标签/状态）对全部匹配结果执行批量操作仍属于后续范围；阶段二 A 继续限定为页面勾选的 ID 集合。
- 批量操作接入 idempotency，单批设上限。

**前端**

- `KeysView.vue` 已增加标签筛选、标签展示、单把创建 / 编辑标签、批量创建统一标签，以及批量标签 set / add / remove / clear。
- 批量创建结果表格和 CSV 导出已包含 `tags` 字段。
- 阶段二 A 暂不抽象独立标签编辑器组件；当前先使用逗号 / 换行分隔输入，后续若需要标签建议、颜色或聚合视图再单独抽组件。
