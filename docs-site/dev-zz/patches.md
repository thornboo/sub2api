...
## 2026-09-08 - 上游 main 同步：分组白名单与网关兼容

- 吸收上游 `b7dba6267`，原 `models_list_config` 升级为 `model_allowlist`，同时控制模型目录和请求准入；保留企业成员按实际候选组执行的路由与预算边界。
- 固定账号 Codex 目录、Astra 元数据、工具约束、WebSocket 状态隔离与流式诊断同步上游；成本证据继续与客户报价分离，分时计费继续使用单一 `TimePricing`。
- 保留现行模型广场与 stone / emerald 控制台，不恢复已替代的旧报价表组件；本地原有 `.gitignore` 修改不进入合并提交，fork 版本保持 `1.7.44`。
- 冲突与最终验证证据见 [上游合并记录](./maintenance/merge-log.md)。

## 2026-09-07 - Key 查询页公告

- 接入现有公告发布、有效期和定向规则，通过 Key 查询会话读取公告和标记已读，不开放用户公告接口给匿名访问。
- Key 已读记录与登录用户已读隔离；界面遵循弹窗／静默设置，并在退出、身份变化或会话失效时清理旧公告请求和队列。
- 公告响应仅返回内容及用户可见字段，沿用安全 Markdown 渲染。详见 [Key 查询页公告合同](./features/public-key-usage-portal.md)。

## 2026-09-07 - 模型状态优先级探测链路

- 任务从账号级改为分组模型轮次，使用账号管理优先级，首个成功终止；已失败再由备用成功为降级可用，未探测和失去资格的跳过分别记录。
- 新增管理员只读链路接口和卡片详情面板，当前配置与历史名称／优先级／步骤分开展示；普通用户 DTO 不增加内部账号信息。
- 迁移 `237_model_self_check_probe_rounds.sql` 增加真实轮次证据表，不改写既有历史。补齐国产平台探测执行分支。
- 验证范围及服务启停边界见[实施记录](./features/model-status-priority-probe-chain.md)。

## 2026-09-07 - 模型状态 V1 账号资格与历史统计一致性

- 问题：自检忽略部分正常调度资格，导致无可用账号仍显示健康；列表按当前账号集合计算历史，和详情快照指标不一致。
- 实现：复用 `IsSchedulableForModelWithContext` / `parentHealthyForShadow`；补齐父账号读取和缺失详情的保守处理；执行前失去资格跳过探测，内部读取失败不污染上游失败历史。
- 历史：新增 `ListStatusSnapshotMetrics`，一次 SQL 对精确分组模型计算 24h/7d/30d 指标；仅确认无快照时保留旧历史回退，未知存储状态不回退。详情不再用历史快照覆盖当前账号探测元数据。
- 保留：用户鉴权与脱敏 DTO、历史数据、10 分钟有效期、平台分支、混合调度现状；平台遗漏、混合调度和有效期配置冲突仍分别跟踪。
- 验证与未执行边界见[实施记录](./features/model-status-v1-bug-analysis-20260907.md)。无迁移、依赖、提交或部署操作。

## 2026-09-05 - 上游 main 同步：模型目录、推理计费与上游请求证据

### 目标与变化

- 合并 `main@578785ee7` 到 `dev-zz`，保留 fork `1.7.44` 与已记录的企业成员、公开目录、单一 TimePricing 和供应商成本边界。
- 吸收 Astra 能力同步与续聊、ultrafast、固定账号 Codex manifest、max 推理倍率、价格文件热重载、账号 lite 列表、上游请求标识和网关 / WS / Claude / 图片 / 支付兼容修复。
- 四份追加迁移按完整文件名并存；同号不重编号，非事务索引按既有 `_notx.sql` runner 处理。

### 合并取舍

- 35 个冲突路径按字段与控制流合流；4 个 modify/delete 维持已替代旧定价测试与模型广场组件的删除状态，适用的新字段进入当前实现。
- usage 统一 73 列 SQL 参数与扫描顺序，保留企业成员、路由快照、上游应扣成本币种 / 参考汇率，并加入管理员可见的直接上游请求 ID。
- max 推理倍率随实际发送档位计费；客户分时规则与最终成功账号绑定价目表的应扣成本分开解析。
- 工具发现继续走请求级 `ResponsesToolRegistry`，显式 client 搜索才映射为 Chat 工具，不为兼容新增测试恢复旧入口或弱化跨来源 identity 隔离。
- 自动合并遗漏的企业成员分组 manifest 快照投影已补齐，JSON 往返继续保留固定账号顺序，成员 Key 的实际分组仍由当前请求选定。
- Anthropic 会话槽释放责任提前到选号结果返回后，覆盖排队、等待、拦截与准入早退；上游实际服务成功后仍保留原有会话续用语义。
- Ent 按最终 schema 重新生成；完整验证与未验证范围见 [本次合并记录](./maintenance/merge-log.md#_2026-09-05-同步上游-main-模型能力-请求证据-推理计费与网关兼容)。

## 2026-09-05 - 用户钱包余额耗尽趋势预测

### 目标

- 在现有用户余额消费趋势中给管理员提供可解释的余额续航估算，辅助判断用户何时可能需要充值。
- 保持钱包余额、订阅额度、标准消费和上游成本口径分离，不新增后端接口或数据库状态。

### 主要变化

- 当前可用余额与“仅钱包余额”口径一起收拢到用户信息区；下方四张等宽卡片依次展示今日消费、周期消费、日均消费和余额预测，避免当前余额重复占用指标卡片。
- 余额预测卡片根据当前可用余额和所选 7、30、90 个自然日的钱包扣款日均值计算预计剩余天数与耗尽日期。
- 时间范围切换和手动刷新会同步更新预测；余额耗尽、不足一天、无消费和加载中均有独立文案，中英文同步覆盖。
- 预测公式提取为无副作用前端工具，并以固定日期覆盖常规、无消费、余额耗尽和不足一天边界。

### 口径与兼容性

- 预测仅使用 `billing_type=0` 的钱包扣款趋势及用户当前 `balance`，不计订阅额度消费，不使用标准 API 原价或供应商上游成本。
- 剩余天数向上取整，预计日期以当前趋势区间结束日为起点；结果是趋势估算，不作为确定的扣费或充值承诺。
- 复用现有趋势接口、时间范围和 stone / emerald 控制台视觉，不涉及 API、数据库迁移、配置或依赖变化。

### 验证

- 前端 ESLint、typecheck、305 个测试文件 / 2121 条用例和生产构建通过。
- 余额预测工具与弹窗回归覆盖 7 / 30 天联动、固定日期计算和无消费边界；`git diff --check` 通过。

### 未验证

- 按用户要求未运行 Playwright 或浏览器截图视觉验收；实际暗色弹窗效果由用户在本地数据环境中确认。
