...
阶段 1 只落地后端兼容层：旧账号级充值记录接口仍保留，但账号已有 active 成本绑定时会读取/写入对应资金池账本。本期账本只支持 `recharge` / `bonus` / `adjustment` 三类非负金额记录，且只有 `recharge` / `bonus` 会生成成本快照。普通用户侧接口不得返回供应商、资金池、上游余额、真实成本或利润字段。

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `GET` | `/api/v1/admin/upstream-suppliers` | 列出上游供应商，阶段 1 至少包含“未归类供应商” |
| `GET` | `/api/v1/admin/upstream-cost-pools` | 列出资金池，返回供应商、当前基础成本、绑定账号数和账本记录数 |
...
| `POST` | `/api/v1/admin/accounts/:id/recharge-records` | 兼容旧入口；写入账号 active 资金池，没有绑定时创建账号默认资金池 |

`PUT /accounts/:id/upstream-cost-binding` 请求主体：
...
```json
{
  ...
  "model_families": [
    { "family": "sonnet", "group_multiplier": 0.8, "note": "供应商折扣" }
  ],
  "note": "绑定到供应商主余额池"
}
```

`POST /upstream-cost-pools/:pool_id/recharge-records` 沿用旧充值记录字段：`type`、`paid_amount`、`paid_currency`、`received_credit_amount`、`received_credit_currency`、`reference_fx_rate`、`recorded_at`、`note`；阶段 1 只接受 `recharge` / `bonus` / `adjustment`，仍固定支持 CNY 实付和 USD 上游额度，金额必须非负。`adjustment` 只作为账本调整记录保存，不刷新资金池当前成本快照。
