...
- 分组 API / DTO 的 `models_list_config` 更名为 `model_allowlist={enabled,models}`；启用后同时约束模型列表与携带模型的请求，按客户端原始模型名判断，不能通过渠道映射绕过。
- 模型条目支持精确名称与末尾 `*`；管理端拒绝启用空列表或中间通配符。旧配置经迁移原样保留，启用旧展示列表的分组现在也会执行调用限制。
- 企业成员继续按请求级实际候选分组执行准入与模型目录过滤，保留 owner/admin 字段隔离与最终分组归因。
...
| 方法 | 路径 | 用途 | 关键语义 |
| --- | --- | --- | --- |
| `GET` | `/api/v1/key/announcements` | 列出当前 Key 可见公告 | 必须持有有效查询 Cookie；沿用所属用户的余额／有效订阅定向，返回 Key 自己的已读状态 |
| `POST` | `/api/v1/key/announcements/:id/read` | 标记当前 Key 已读 | 会话解析 Key 身份，重新校验公告可见性，幂等写入；不修改登录用户或其他 Key 的已读状态 |

公告内容、通知模式、时间及已读字段采用用户可见 DTO，不返回管理员定向条件或 Key 所属用户信息。未输入 Key 的匿名页面不加载公告。详见 [Key 查询页面合同](../features/public-key-usage-portal.md)。

创建／恢复有效查询会话时返回非凭据 `session_id`，用于同一标签页跨路由的公告展示去重。它与 Cookie 令牌及服务端缓存查询哈希隔离，不能替代 Cookie 鉴权；无效会话不返回该标识。
...
用户侧模型服务状态要求登录用户身份，只在**当前用户可用分组**内按 **分组 / 模型** 维度展示公开模型健康状态。可见范围复用 API Key 分组绑定口径：公开标准分组可见，专属分组需要明确授权，订阅分组需要有效订阅。数据来源是**站点自检探针**：对在渠道定价中开启「自检」的模型，系统用合成请求走本站网关真实链路（标记为探针，**不写用量、不计费、不触发生产账号封禁/重试/failover**），结果写入 `model_self_check_histories`，按 (分组, 模型) 对覆盖账号做 OR 聚合（任一账号成功即视为该分组下模型可用）。与管理员侧「渠道监控」（上游探针，写 `channel_monitor_histories`）完全分离。
