...
- 管理端账号列表新增供应商成本上下文列，并把「供应商、综合折扣、充值/汇率、倍率」放到「分组」列后方，便于管理员在同一行同时查看账号分组和上游成本归属。
...
- 成本池 review follow-up：`adjustment` 只保存账本不刷新当前成本快照；默认资金池创建改为事务内账号级锁；账号成本绑定 `GET` 保持只读；未归类供应商补 active 名称唯一约束。
