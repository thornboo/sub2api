...
- 模型状态 V1 增加管理员优先级探测链路：按分组模型独立执行，账号优先级升序、失败立即切换、首个成功结束；当前状态采用实际轮次证据，备用未探测不再自动导致降级。详情分开显示当前候选和历史步骤，账号身份只通过管理接口返回。新增轮次存储迁移 237，并补齐 Kimi／智谱／DeepSeek 的自检协议分支。详见[实施记录](./features/model-status-priority-probe-chain.md)。

- 修正模型状态 V1 的账号资格偏差：探测任务、当前状态与快照统一复用账号／模型可调度判断，保留 Antigravity overages 和 Spark 母账号凭据边界；失效任务执行前跳过，不再写伪上游失败。
- 列表与详情历史指标统一优先按分组模型快照批量聚合，当前探测元数据只反映合格账号；存储错误不回退成虚假的旧成功可用率。该资格修复阶段保留历史数据和正常业务调度；随后的链路增强另外新增前端详情及迁移。详见[问题分析及实施记录](./features/model-status-v1-bug-analysis-20260907.md)。

## 2026-09-05

- 同步上游 `main@578785ee7` 到正式线 `dev-zz`：吸收 GPT-6 Astra 能力同步与续聊、OpenAI ultrafast 档位、固定账号 Codex 模型目录、GLM 推理映射、图片 URL 回填及 Claude CLI / billing fingerprint 兼容修复；fork 版本保持 `1.7.44`。
- 管理员账号列表支持紧凑响应，上游请求标识可按账号指定响应头记录；渠道模型增加 max 推理倍率，定价 fallback / override 文件支持内容哈希热重载。继续保留供应商成本上下文、企业成员实际分组与预算证据、公开模型目录及单一分时定价。
- 同步失败请求释放会话槽、WebSocket replay 内存共享与加密续聊、模型映射调度、并发拒绝来源、工具发现、支付宝订单对账和零值系统指标修复。工具桥沿用本分支注册表与显式 client 工具搜索边界；上游成本币种和参考汇率证据继续与客户计费隔离。
...
- 用户管理的余额消费趋势弹窗新增钱包余额耗尽预测：按当前选择的 7、30 或 90 个自然日内实际钱包扣款计算日均消费，再用当前可用余额估算剩余天数和预计耗尽日期。切换趋势周期或刷新数据时预测同步更新；余额已耗尽、预计不足一天和所选周期无钱包消费均使用明确状态，不把订阅额度、标准价或上游成本混入预测。
